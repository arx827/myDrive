    ...
    >
      <template v-slot:body>
        ...
        <div v-else>
          親愛的護理同仁您好，
          <br>
          您欲發送的項目，因{{ msg }}因素無法進行。
          <br>
          如尚需發送通知，請您再次嘗試。或與相關服務人員聯繫，謝謝您！
        </div>
      </template>
      <template v-slot:buttons>
        <router-link to="/occupationSafety/ErgonomicHazard/QueryData/list">
          <a
...
export default class QueryDataResult extends Vue {
  ...
  created() {
  	...
  	if (this.result === 'success') {
  		this.formCnt = query.data.formCount;
  		this.healthCnt = query.data.healthCount;
  		this.phyConsultCnt = query.data.phyConsultCount;
  	} else {

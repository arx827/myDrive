  ...
  <div>
    ...
    >
      ...
      <div class="btn__wrap text-center">
        <router-link :to="'/occupationSafety/MaternalProtect/eventContentMaintain/index'">
...
import { MonPlanConfigListDto } from '@fubonlife/oss-api-axios-sdk';
...
export default class EventContentMaintainEdit extends Vue {
  ...
  handleChange(e) {
  	...
  	if (e.file.status == 'removed') {
  		...
  		this.imgData = null;
  		// console.log('remove', this.uploadedFileList);
  	}
  ...
  uploadImg(file) {
  	...
  	// this.$MONPLANRpnEventContentMaintainApi.uploadMonPlanCasePictureUsingPOST(file)
  	// 	.then((resp) => {
  ...
  created() {
  	...
  	this.content = query;
  	console.log(typeof (query[1].value));
  	this.description = query[1].value;
  	this.personalInfoStatement = query[2].value;
  	if (!query[0].value) return;
  	this.imgData = query[0].value;
  ...
  submit() {
  	const newContent: MonPlanConfigListDto = {
  		monPlanConfigDtoList: [
  			{
  				code: this.content[0].code,
  				note: this.content[0].note,
  				value: this.filePath,
  			},
  			{
  				code: this.content[1].code,
  				note: this.content[1].note,
  				value: this.description,
  			},
  			{
  				code: this.content[2].code,
  				note: this.content[2].note,
  				value: this.personalInfoStatement,
  			},
  		],
  	};
  	this.setLoading(true);
  	this.$customUpload.fetchAPI(
  		UploadAPIUrl.MOTHEREVENT,
  		'monPlanConfigListDto',
  		newContent,
  		'file',
  		[this.files],
  		(resp) => {
  			this.$global.changeRouterAndaddParam({
  				toRouter: 'EventContentMaintainResult',
  				query: {
  	...
  	// 	this.setLoading(true);
  	// 	this.$MONPLANRpnEventContentMaintainApi.updateEventContentRUsingPOST(newContent)
  	// 		.then((resp) => {

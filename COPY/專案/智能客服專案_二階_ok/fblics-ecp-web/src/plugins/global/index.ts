...
export class GlobalService {
  ...
  /**
   * 幫某個路由 追加 query資料 (不跳轉)
   * @param {string} toRouter: 要追加資料的路由名稱
   * @param {any} query: 欲夾帶的query資料
   */
  ...
  public getParam(): { fromPage: string; query: any } | null {
    ...
    if (!Object.keys($param).includes($currentName)) return null
  ...
  public returnCardImgPath = cardImgPath => {
    return `${import.meta.env.VITE_BASE_URL.slice(0, -1)}${cardImgPath}`
  }

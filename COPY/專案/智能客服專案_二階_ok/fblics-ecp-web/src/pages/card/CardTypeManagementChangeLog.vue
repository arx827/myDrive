...
import type { CardTypeManagementChangeLog } from '@pages/page_modal'

const dateFormat: string = 'YYYY/MM/DD'

const {
  proxy: { $global, $fblicsEnum, $cardCategoryManagementApi, $dateTimeUtil },
} = getCurrentInstance()

const { setLoading } = useLoadingStore()

const formData = ref<CardTypeManagementChangeLog.formDataType>({
  changePersonnel: '',
 ...
 */
// API: 查詢卡片類型異動紀錄
const postAPI_findCardCategoryLogs = async () => {
  setLoading(true)
  const $submit = {
  ...
  await $cardCategoryManagementApi
    .findCardCategoryLogsUsingPOST($submit)
    .then(res => {
...
onMounted(async () => {
  initDate()
  await postAPI_findCardCategoryLogs()
})
</script>
<template>
  <FormTitle title="卡片類型管理-變更紀錄" />

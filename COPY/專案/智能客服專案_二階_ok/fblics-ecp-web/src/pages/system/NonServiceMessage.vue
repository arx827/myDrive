...
import type { NonServiceMessage } from '@pages/page_modal'
...
const formData = ref<NonServiceMessage.formDataType>({
  message: '',
})

const formRules = ref<{ [k: string]: RuleObject | RuleObject[] }>({
  message: [{ required: true, message: '請輸入 非服務時段宣告文字訊息' }],
})
 ...
 */
// API: 非服務時段宣告文字訊息
const getAPI_getNoServiceMessage = () => {
  setLoading(true)
  $serviceSettingsApi
    .getNoServiceMessageUsingPOST()
    .then(res => {
 ...
 */
onMounted(() => {
  getAPI_getNoServiceMessage()
})
</script>
<template>
  <FormTitle title="非服務時段宣告文字訊息" />
  <a-form :model="formData" name="basic" autocomplete="off" layout="vertical">
    <a-form-item label="非服務時段宣告文字訊息(支援HTML)" v-bind="validateInfos.message">

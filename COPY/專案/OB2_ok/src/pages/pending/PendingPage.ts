...
import { PendingFilter, PendingGrid, PendingListDto, StaffDto, TSysCodeDto,ComponentDto, AllUserUnitSuperUnitInfoDto, Option, CasePolicyAndLogIdDto,
    QuestAreaDto } from "@fubonlife/obd-api-axios-sdk";
...
import { QuestAllData } from "./model";
import BlockHeader from '@/components/shared/block-header/BlockHeader.vue';
import QuestOpenArea from "@/pages/questOpenAreaPreview/QuestOpenArea.vue"; //新建preview用元件
import CustomerAnswer from "@/components/shared/customerAnswerPreview/CustomerAnswer.vue"; //新建preview用元件
import QuestCheckIdArea from "@/pages/questCheckIdAreaPreview/QuestCheckIdArea.vue"; //新建preview用元件
import QuestArea from "@/pages/questAreaPreview/QuestArea.vue"; //新建preview用元件

@Component({
    components: { FblDataGrid, HiddenFolde, TimePicker, ModifyCallDateForm, PolicyMaskForm,
        BlockHeader,QuestOpenArea,CustomerAnswer, QuestCheckIdArea, QuestArea}
})
export default class PeddingPage extends Vue {
    ...
    peddingSearchForm: PeddingSearchForm = {
        ...
        agentName: "",
        isLegal: "",
        caseCloseStartDate: null,
        datePickerCaseCloseStartDate: null,
        caseCloseEndDate: null,
        datePickerCaseCloseEndDate: null,
        caseCloseStartDateString: "",
        caseCloseEndDateString: "",
    };
    ...
    isCaseCloseStartDateVisible: boolean = false;
    isCaseCloseEndDateVisible: boolean = false;
    ...
    peddingSearchValidateForm: PeddingSearchValidateForm = {
        ...
        caseCloseStartDate: { feedback: false, hoverVisible: false },
        caseCloseEndDate: { feedback: false, hoverVisible: false },
    }
    ...
    peddingSearchRules: { [key: string]: ValidationRule[] } = {
        ...
        caseCloseStartDate: [{ validator: this.validateCaseCloseStartDate, trigger: "blur" }],
        caseCloseEndDate: [{ validator: this.validateCaseCloseEndDate, trigger: "blur" }],
    };
    ...
    peddingFilter: PendingFilter = {
        ...
        importDateEnd: null,
        dueContDateChgStart: null,
        dueContDateChgEnd: null,
        contactDateStart: null,
        contactDateEnd: null,
        obdStage: "",
        obdStatus: "",
        policyNo: "",
        ...
        custId: "",
        changeNo: "",
        caseCloseDateStart: null,
        caseCloseDateEnd: null,
    }

    //--------------------------------------------問卷預覽資料容器與參數區start-----------------------------------------
    idShowQuestPreviewModal:boolean=false;
    //=====問卷區塊參數=====
    questTitle = "電訪問卷";
    /** 問卷各區塊答案資料 */
    ...
    questTipMsg: string = "<font style='color:#FAAD14'>★</font>：符合電話變更作業　" +
        "<font style='color:#FC0518'>！</font>：點選的答案包含「否」或「不確定」";
    ...
    grid: FblPDataGridHolder<PendingGrid> = {
        ...
        columns: [
            ...
            {
                type: FblColumnType.TEMPLATE, //為推上prod先改為Template
                title: this.$t('case_search_grid_questName').toString(), //問卷名稱
                width: 100,
                align: 'center',
                template: 'questName', //綁定.vue中的template樣式
                //hidden: true //為推上prod先隱藏
            ...
            },
            {
                ...
                formatter: (data: PendingGrid) => {
                    if (data.agentAssignStartTime == null && data.agentAssignEndTime == null) {
                        return "";
                    }else if(data.agentAssignStartTime == '' && data.agentAssignEndTime == ''){
                        return "";
            ...
            },
            {
                ...
                formatter: (data: PendingGrid) => {
                    if (data.visitStartDate == null && data.visitEndDate == null) {
                        return "";
                    } else if(data.visitStartDate == '' && data.visitEndDate == ''){
                        return "";
            ...
            },
            {
                ...
                title: this.$t('pedding_callFreqDay').toString(), //已電訪天數
                width: 90,
                align: 'center',
                formatter: (data: any) => {
                    if (VlidationUtil.isEmpty(data.callFreqDay)) {
                        if (VlidationUtil.isEmpty(data.limitCaseNo)) {
                            return "NA";
                        }else{
                            return "0";
                        }
                    } else {
                        return data.callFreqDay;
                    }
                }
            },
            {
                ...
                title: this.$t('pedding_allFreqVisit').toString(), //已電訪次數
                width: 90,
                align: 'center',
                formatter: (data: any) => {
                    if (VlidationUtil.isEmpty(data.callFreqVisit)) {
                        if (VlidationUtil.isEmpty(data.limitCaseNo)) {
                            return "NA";
                        }else{
                            return "0";
            ...
            },
            {
                ...
                title: this.$t('pedding_caseCloseDate').toString(), //結案日期
                formatter: (data: PendingGrid) => {
                    if (data.caseCloseDate == null) {
                        return "";
                    } else {
    ...
    peddingSearch() {
        if (this.validateSearch(true)) {
            ...
            const caseCloseDateStart = FiltersUtil.setFilterParam("caseCloseDate", FblOperator.GEQ, moment(this.peddingSearchForm.caseCloseStartDate).startOf("day").toISOString(true));
            const caseCloseDateEnd = FiltersUtil.setFilterParam("caseCloseDate", FblOperator.LEQ, moment(this.peddingSearchForm.caseCloseEndDate).endOf("day").toISOString(true));
            ...
            this.peddingFilter.importDateStart = (this.peddingSearchForm.importStartDate == null) ? null : moment(this.peddingSearchForm.importStartDate).startOf("day").toISOString(true);
            this.peddingFilter.importDateEnd = (this.peddingSearchForm.importEndDate == null) ? null : moment(this.peddingSearchForm.importEndDate).endOf("day").toISOString(true);
            this.peddingFilter.dueContDateChgStart = moment(this.peddingSearchForm.dueContactStartDate).startOf("day").toISOString(true);
            this.peddingFilter.dueContDateChgEnd = moment(this.peddingSearchForm.dueContactEndDate).endOf("day").toISOString(true);
            this.peddingFilter.contactDateStart = (this.peddingSearchForm.contactDates[0] == null) ? null : MomentUtil.transferMomentToTimestamp(this.peddingSearchForm.contactDates[0]).toString();
            this.peddingFilter.contactDateEnd = (this.peddingSearchForm.contactDates[1] == null) ? null : MomentUtil.transferMomentToTimestamp(this.peddingSearchForm.contactDates[1]).toString();
            this.peddingFilter.obdStage = this.peddingSearchForm.caseStageId;
            this.peddingFilter.obdStatus = this.peddingSearchForm.caseStatusId;
            this.peddingFilter.policyNo = this.peddingSearchForm.policyNo01;
            this.peddingFilter.policySeq = this.peddingSearchForm.policyNo02;
            this.peddingFilter.idDup = this.peddingSearchForm.policyNo03;
            this.peddingFilter.custId = this.peddingSearchForm.respondentsId.toUpperCase();
            this.peddingFilter.changeNo = this.peddingSearchForm.changeNo;
            this.peddingFilter.caseCloseDateStart = moment(this.peddingSearchForm.caseCloseStartDate).startOf("day").toISOString(true);
            this.peddingFilter.caseCloseDateEnd = moment(this.peddingSearchForm.caseCloseEndDate).endOf("day").toISOString(true);
            ...
            this.peddingSearchFilters = FiltersUtil.setFilters(importDateStart, importDateEnd, dueContDateChgStart, dueContDateChgEnd, obdStage, obdStatus, policyNo, policySeq, idDup, custId,
                changeNo, contactDateStart, contactDateEnd, caseCloseDateStart, caseCloseDateEnd);
    ...
    validateSearch(isRealSearch) {
        ...
        let validateCaseCloseDate = true;
        ...
        this.validateCaseCloseStartDate(null, this.peddingSearchForm.caseCloseStartDateString, () => {
            if (this.peddingSearchValidateForm.caseCloseStartDate.feedback) {
                validateCaseCloseDate = false;
                validate = false;
            }
        });
        this.validateCaseCloseEndDate(null, this.peddingSearchForm.caseCloseEndDateString, () => {
            if (this.peddingSearchValidateForm.caseCloseEndDate.feedback) {
                validateCaseCloseDate = false;
                validate = false;
            }
        });
        ...
        if (validateCaseCloseDate && !ValidationUtil.isEmpty(this.peddingSearchForm.caseCloseStartDateString) && !ValidationUtil.isEmpty(this.peddingSearchForm.caseCloseEndDateString)) {
            this.validateCaseCloseStartAndEndDate(null, this.peddingSearchForm.datePickerCaseCloseStartDate, this.peddingSearchForm.datePickerCaseCloseEndDate, this.peddingSearchForm.caseCloseStartDateString,
                this.peddingSearchForm.caseCloseEndDateString, this.isCaseCloseStartDateVisible, this.isCaseCloseEndDateVisible, () => {
                    if (this.peddingSearchValidateForm.caseCloseStartDate.feedback || this.peddingSearchValidateForm.caseCloseEndDate.feedback) {
                        validate = false;
                    }
                });
        }
        ...
        if (validate && isRealSearch) {
            ...
            if(VlidationUtil.isEmpty(this.peddingSearchForm.importStartDate) && VlidationUtil.isEmpty(this.peddingSearchForm.importEndDate)
             ...
             && VlidationUtil.isEmpty(this.peddingSearchForm.caseCloseStartDate) && VlidationUtil.isEmpty(this.peddingSearchForm.caseCloseEndDate)
             && VlidationUtil.isEmpty(this.peddingSearchForm.policyNo01) && VlidationUtil.isEmpty(this.peddingSearchForm.changeNo)){
                validate = false;
                ErrorModalUtil.modalError(this.$t('pedding_importOrDueContactDateRequire').toString()); //查詢條件：匯入日期、應電訪日、結案日期 需擇一填寫
            }
        }
    ...
    checkFilterCount() {
        ...
        if(!isAbleToSearch){
            ...
            if(filterColumn.some((c)=> c == "caseCloseStartDateString" || c == "caseCloseEndDateString" )){
                count ++;
                filterColumn = filterColumn.filter((c)=> c != "caseCloseStartDateString" && c != "caseCloseEndDateString" && c != "datePickerCaseCloseStartDate" && c != "datePickerCaseCloseEndDate" );
            }
    ...
    pendingTodayClick() {
        ...
        this.peddingFilter.contactDateStart = moment(new Date()).startOf("day").toISOString(true);
        this.peddingFilter.contactDateEnd = moment(new Date()).endOf("day").toISOString(true);
        this.peddingFilter.tmrIdList = [LoginModule.loginState.me.id];
        this.peddingFilter.departmentIdList = [];
    ...
    pendingPickUp() {
        ...
        this.$packMatchApi.queryCasePackTempUsingGET(LoginModule.loginState.me.id).then((resp) => {
            ...
            if (resp.data.success) {
                ...
                // 更新契約狀態
                if (resp && resp.data && resp.data.firstCasePack) {
                    const casePolicyAndLogIdDtoList = resp.data.firstCasePack.casePolicyLogInfoList.map(c => { return {caseNo: c.caseNo, caseLogId: c.guid} as CasePolicyAndLogIdDto;});
                    if (casePolicyAndLogIdDtoList != null && casePolicyAndLogIdDtoList.length > 0) {
                        this.$policyDataApi.getPolicyContractStatusDataUsingPOST(casePolicyAndLogIdDtoList)
                        .then(()=> this.$router.push("/on-duty"))
        ...
        this.$packMatchApi.queryAndMatchCasePackUsingGET().then((resp)=>{
            if(!resp.data.success){
                ...
                if(resp.data.firstCasePack!= null){
                    ...
                    ContinuePackMoudle.initContinuePackInfo();
                    // 更新契約狀態
                    if (resp && resp.data && resp.data.firstCasePack && resp.data.firstCasePack.casePolicyLogInfoList) {
                        const casePolicyAndLogIdDtoList = resp.data.firstCasePack.casePolicyLogInfoList.map(c => { return {caseNo: c.caseNo, caseLogId: c.guid} as CasePolicyAndLogIdDto;});
                        if (casePolicyAndLogIdDtoList != null && casePolicyAndLogIdDtoList.length > 0) {
                            this.$policyDataApi.getPolicyContractStatusDataUsingPOST(casePolicyAndLogIdDtoList)
                            .then(()=> this.$router.push("/on-duty"))
    ...
    clearSearchFrom() {
        ...
        this.peddingSearchForm.caseCloseStartDate = null;
        this.peddingSearchForm.datePickerCaseCloseStartDate = null;
        this.peddingSearchForm.datePickerCaseCloseEndDate = null;
        this.peddingSearchForm.caseCloseEndDate = null;
        this.peddingSearchForm.caseCloseStartDateString = "";
        this.peddingSearchForm.caseCloseEndDateString = "";
    }
    ...
    clearValidateStatus() {
        ...
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
    }
    ...
    //===========================結案日起訖 相關方法 start ==========================================
    //自動轉為字串更新搜尋條件
    onCaseCloseStartDateChange(date) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        this.peddingSearchForm.caseCloseStartDateString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(date)));
        this.peddingSearchForm.caseCloseStartDate = date;
        this.isCaseCloseStartDateVisible = false;
        this.isCaseCloseEndDateVisible = false;
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
        this.validateSearch(false);
    }
    ...
    //自動轉為字串更新搜尋條件
    onCaseCloseEndDateChange(date) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        this.peddingSearchForm.caseCloseEndDateString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(date)));
        this.peddingSearchForm.caseCloseEndDate = date;
        this.isCaseCloseStartDateVisible = false;
        this.isCaseCloseEndDateVisible = false;
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
    ...
    //清除日期
    clearCaseCloseDate(cleanType: String) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
        ...
        this.isCaseCloseEndDateVisible = false;
        if(cleanType === DatePickerTypeEnum.StartDate) {
            this.peddingSearchForm[DatePickerEnum.caseCloseStartDateString] = "";
            this.peddingSearchForm[DatePickerEnum.caseCloseStartDate] = null;
            if(!VlidationUtil.isEmpty(this.peddingSearchForm.caseCloseEndDate)){
                this.isCaseCloseStartDateVisible = true;
                CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, true, this.$t('pedding_caseCloseStartDateRequire').toString(), false); //結案日(起) 必填
            }
        } else if (cleanType === DatePickerTypeEnum.EndDate) {
            this.peddingSearchForm[DatePickerEnum.caseCloseEndDateString] = "";
            this.peddingSearchForm[DatePickerEnum.caseCloseEndDate] = null;
            if(!VlidationUtil.isEmpty(this.peddingSearchForm.caseCloseStartDate)){
                this.isCaseCloseEndDateVisible = true;
                CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, true, this.$t('pedding_caseCloseEndDateRequire').toString(), false); //結案日(訖) 必填
            }
        }
    }
     ...
     * @param data 
     */
    checkManualInputCaseCloseStartDate(data: any) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        this.isCaseCloseStartDateVisible = false;
        this.isCaseCloseEndDateVisible = false;
        ...
        if (parseDate) {
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
        this.peddingSearchForm.datePickerCaseCloseStartDate = parseDate;
        } else {
            this.isCaseCloseStartDateVisible = true;
            // 日期錯誤
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, true, this.$t('global_dateError').toString(), false);
        }
        this.peddingSearchForm.caseCloseStartDate = parseDate ? parseDate : this.peddingSearchForm.caseCloseStartDate;
        this.peddingSearchForm.caseCloseStartDateString = parseDate ?
            MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(this.peddingSearchForm.caseCloseStartDate.toString()))) :
            data.currentTarget.value.match(/\d+\/\d+\/\d+/)[0];
    }
     ...
     * @param data 
     */
    checkManualInputCaseCloseEndDate(data: any) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        this.isCaseCloseStartDateVisible = false;
        this.isCaseCloseEndDateVisible = false;
        ...
        if (parseDate) {
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
            this.peddingSearchForm.datePickerCaseCloseEndDate = parseDate;
        } else {
            this.isCaseCloseEndDateVisible = true;
            // 日期錯誤
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, true, this.$t('global_dateError').toString(), false);
        }
        this.peddingSearchForm.caseCloseEndDate = parseDate ? parseDate : this.peddingSearchForm.caseCloseEndDate;
        this.peddingSearchForm.caseCloseEndDateString = parseDate ?
            MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(this.peddingSearchForm.caseCloseEndDate.toString()))) :
            data.currentTarget.value.match(/\d+\/\d+\/\d+/)[0];
    }
    ...
    //勾選狀態改變
    handleCheckAllSelection(selectedRows) {
        // console.log('selectedRows: ',selectedRows);
        //清空caseNumbers&pendingCasesMapStatus
        this.caseNumbers = new Set();
        this.pendingCasesMapStatus = new Map<number, boolean>();
        //重新加入相關資料
        for(let selectedRow of selectedRows){
            this.caseNumbers.add(parseInt(selectedRow.caseNo));
            this.$pendingPageApi.judgeCaseFinishedyetUsingGET(selectedRow.caseNo)
                .then((resp) => {
                    let flag = resp.data;
                    this.pendingCasesMapStatus.set(parseInt(selectedRow.caseNo), flag);
                }).catch(() => {
                    ErrorModalUtil.modalError(this.$t("judge_cases_fiinished_yet").toString());
                })
        }
    ...
    * @returns 
    */
    validateCaseCloseStartDate(rule, value, callback) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        if (!ValidationUtil.isEmpty(value)) {
            ...
            if (parseDate) {
                ...
                this.isCaseCloseStartDateVisible = true;
                CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, true, this.$t('global_dateError').toString(), false); //日期錯誤
                callback(() => { });
            }
        }else if(!VlidationUtil.isEmpty(this.peddingSearchForm.caseCloseEndDate)){
            this.isCaseCloseEndDateVisible = true;
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, true, this.$t('pedding_caseCloseStartDateRequire').toString(), false); //結案日(起) 必填
            callback(() => { });
        }
    ...
    * @returns 
    */
    validateCaseCloseEndDate(rule, value, callback) {
        this.isExportDisable = true;
        this.overMaxRowCountMessage = "";
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
        if (!ValidationUtil.isEmpty(value)) {
            ...
            if (parseDate) {
                ...
                this.isDueContactEndVisible = true;
                CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, true, this.$t('global_dateError').toString(), false); //日期錯誤
                callback(() => { });
            }
        }else if(!VlidationUtil.isEmpty(this.peddingSearchForm.caseCloseStartDate)){
            this.isCaseCloseEndDateVisible = true;
            CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, true, this.$t('pedding_caseCloseEndDateRequire').toString(), false); //應電訪日(訖) 必填
            callback(() => { });
        }
        callback();
    }
    ...
    * @returns 
    */
    validateCaseCloseStartAndEndDate(rule, startDate, endDate, startString, endString, dateDisableStart, dateDisableEnd, callback) {
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, false, '', false);
        CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, false, '', false);
        if (startString == endString || moment(startDate).isBefore(endDate)) {
            var isOver31Days = MomentUtil.betweenStartAndEndOverTime(moment(startDate), moment(endDate), 'days', 31);
            if(!isOver31Days){
                ...
                dateDisableStart = true;
                dateDisableEnd = true;
                if (!ValidationUtil.isEmpty(this.peddingSearchForm.caseCloseStartDateString) && !ValidationUtil.isEmpty(this.peddingSearchForm.caseCloseEndDateString)) {
                    CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, true, this.$t('pedding_startAndEndOver31_due').toString(), false); //起訖不可超過31天
                    CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, true, this.$t('pedding_startAndEndOver31_due').toString(), false); //起訖不可超過31天
                }
                callback(() => { });
            }
        } else {
            ...
            dateDisableEnd = true;
            if (!ValidationUtil.isEmpty(this.peddingSearchForm.caseCloseStartDateString) && !ValidationUtil.isEmpty(this.peddingSearchForm.caseCloseEndDateString)) {
                CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseStartDate, true, this.$t('global_pleaseInputCorrectStartAndEndDate').toString(), false); //請輸入正確的起訖日期
                CommonUtil.feildValidateWithVisible(this.peddingSearchValidateForm.caseCloseEndDate, true, this.$t('global_pleaseInputCorrectStartAndEndDate').toString(), false); //請輸入正確的起訖日期
            }
            callback(() => { });
        }
        callback();
    ...
    */
    ...
    getQuestName(caseNo){
        ...
        formData.append("caseNo",caseNo)
            axios(
                {
                method:'post',
                url:`${process.env.VUE_APP_API_BASE_URL}/api/questMain/getQuestName`,
                data: formData,
                }
            ).then(Response=>
                {
                console.log("Response.data");
                questName=Response.data;
                return "questName"; 
                } 
            )
    ...
    packNoTest: string = "";
    //取得問卷資料但不存檔
    questPreview(packNo,caseNo){
        console.log('packNo: '+packNo)
        console.log('caseNo: '+caseNo)
        this.idShowQuestPreviewModal=true;
        //開啟讀取
        LoadingUtil.show();
        let formData = new FormData;
        formData.append("packNo",packNo);
        this.packNoTest=packNo;
        axios(
          {
            method:"post",
            url: `${process.env.VUE_APP_API_BASE_URL}/api/questMain/getQuestDataWithOutSaving`,
            data:formData
          }
        ).then(resp=>{
            console.log("reponse物件為")
            console.log(resp.data);
            console.log("問卷區塊資料")
            console.log(resp.data.questAreaData)
    
            //let dto=resp.data.questAreaData;
            //this.openAllArea();
            ...
            this.questAreaData = resp.data.questAreaData;//問卷區塊
    ...
    onCloseModal() {
        this.idShowQuestPreviewModal = false; //關閉modal
  ...
  resetQuestAnsData(questAreaData) {
    ...
    questAreaData.questDto.forEach((quest) => {
      console.log("初始化資料");
      let caseNo_Q = quest.caseNo

      //塞入caseNo key
      this.questAllData.answer.policy[caseNo_Q] = {};
      this.questAllData.answer.quest[caseNo_Q] = {};
      this.questAllData.custAnswer.quest[caseNo_Q] = {};
      this.questAllData.custHisAnswer.quest[caseNo_Q] = {};
      this.questAllData.custAnswer.end[caseNo_Q] = "";
      this.questAllData.custHisAnswer.end[caseNo_Q] = "";

      //保單資料答案
      quest.questPolicyDto.forEach((policy) => {
        if (policy.answerOption != null && policy.answerOption != "")
          this.questAllData.answer.policy[caseNo_Q][policy.itemCode] = "";
      });

      //問卷答案、保戶回答、歷程
      quest.questContDto.forEach((qCont) => {
        if (qCont.answerOption != null && qCont.answerOption != "")
          this.questAllData.answer.quest[caseNo_Q][qCont.itemCode] = { value: "" };

        this.questAllData.custAnswer.quest[caseNo_Q][qCont.itemCode] = "";
        this.questAllData.custHisAnswer.quest[caseNo_Q][qCont.itemCode] = "";
      });

      //重寄信函內容
      this.questAllData.answer.reSendLetter[caseNo_Q] = { type: "", addr: "" };

      //是否點過問卷檢核
      this.questAllData.answer.questCheck[caseNo_Q] = { doChecked: "N", pass: "N", refuseCheck: "N" };

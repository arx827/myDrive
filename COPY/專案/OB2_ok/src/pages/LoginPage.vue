...
export default class LoginPage extends Vue {
  ...
  FAIL_FROM_SSO: string = "fail_from_sso";
  ...
  created() {
    ...
    // 如果有有效的storage token就直接到待電訪畫面，以避免各種奇怪的問題，如重複登入等。
    // // 這裡不檢查SSO cookie token，因為點擊登出時無法刪除SSO cookie token，故會再被轉入系統。
    // let validStorageToken = false;
    // if (LoginModule.loginState && LoginModule.loginState.accessToken) {
    //   let jwtStr = LoginModule.loginState.accessToken;
    //   const decoded = jwt.decode(jwtStr);
    //   const now = Date.now();
    //   if (decoded) {
    //     validStorageToken = !(Math.floor(now / 1000) > decoded.exp);
    //   }
    // }

     // 檢查storage token，如果有效就直接跳轉至待電訪頁面
    if (LoginModule.hasValidToken) {
      this.$router.push("/pending-page");
    } else {
      // 如果沒有效的token，就要檢查為什麼沒有有效token，並將相關警訊跳出。
      let userId = "";
      if (LoginModule.loginState && LoginModule.loginState.me && LoginModule.loginState.me.id) {
        userId = LoginModule.loginState.me.id;
      } else if (localStorage.getItem("fail_from_sso")) {
        userId = localStorage.getItem("fail_from_sso");
      }
      // 這裡只是要檢查AD帳號並跳出相關警訊而已，並沒有後續動作。
      if (userId) {
        LoadingUtil.show();
  ...
  // async loginFromSso(validatedInfo: string) {
    ...
    // } else {
      ...
      // await this.validUserCheck(userId);
      // await this.repeatLoginCheck(userId);
    // }
    // LoadingUtil.close();
  // }
  ...
  async login() {
    ...
    if (
      ...
      let loginToken = "";
      await this.$authApi
        .loginForCrowdAndLdapUsingPOST(body)
        .then((resp) => {

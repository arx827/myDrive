<template >
    <div tabindex="-1"  @keyup.enter="userUnlockSearch()">
            ...
            <template v-slot:hiddenArea="hiddenFoldeStyle">
                 <a-form-model
                   :label-col="{ xs: 7, md: 9, xxl: 7 }"
                    ...
                    :model="sendEmailTemplateSearchForm"
                ...
                <!-- 發送對象 -->
                    <a-form-model-item>
                        ...
                        <a-select
                        class="select"
                        v-model="sendEmailTemplateSearchForm.Target"
                        :allowClear="true"
                ...
                <a-col :span="5">
                <a-form-model-item
                    :label="$t('sendEmailTemplateSettingPage_subject')"
                    prop="subject"
                    style="margin-bottom: 0px"
                >
                    <a-input
                    type="text"
                    v-model="sendEmailTemplateSearchForm.subject"
                    />
                </a-form-model-item>
                </a-col>
                ...
                >
                    <a-input
                    type="text"
                    v-model="sendEmailTemplateSearchForm.content"
                    />
                </a-form-model-item>
                </a-col>
                </a-row>
                ...
                <div>
                    ...
                    <a-space style="margin-bottom: 16px">
                        <!-- 查詢 -->
                        <a-button type="primary" @click="sendEMAILTemplateSearch()">{{
                        $t("global_search")
                        }}</a-button>
                        <!-- 清除 -->
                        <a-button type="default" @click="sendEmailTemplateSearchReset()">{{
                        $t("global_clean")
                        }}</a-button>
                    </a-space>
                    </a-row>
        ...
        <a-row>
            <a-col>
                ...
                @click="showSendEmailTemplateAddModal()"
                >
        ...
        >
         <template v-slot:deleteTemp="slotProps">
            <div>
              <a-button v-if="slotProps.data.crud && slotProps.data.crud.includes('D')"
                icon="delete"
    ...
    <!-- 新增範本通知 -->
    <a-modal
      v-model="isSendEMAILTemplateSettingVisible"
      :title="sendEMAILTemplateSettingTitle"
      :closable="true"
      :maskClosable="false"
      :keyboard="false"
      :destroyOnClose="true"
    ...
    >
      ...
      :parentIsEmail = false
       :sendMode = "sendMode"
      @reloadSendEMAILTemplate="isSendEMAILTemplateSettingVisible = false;sendEMAILTemplateSearch();"
      >
      
      </SendTemplateSettingUpdateForm>
       <template #footer>
        <!-- 離開 -->
        <a-button key="button" @click="isSendEMAILTemplateSettingVisible = false">{{
          $t("global_leave")
        }}</a-button>
        <!-- 儲存 -->
        <a-button
...
<script src="./SendEmailTemplateSetting.ts" lang="ts"></script>
...
import ImpairmentModal from "@/components/shared/form/impairment/ImpairmentModal.vue";
...
@Component({
  components: { FblDataGrid, HiddenFolde, CuntersignatureModal, CountersignatureModalVue, DragModal, PhoneChangeModal, SuspectiveModal, NotificationModal ,ImpairmentModal}
})
export default class ReviewCasePage extends Vue {
  ...
  // 聽語障覆核單是否顯示
  isImpairFormVisible: boolean = false;
  ...
  isImpairmentDisable: boolean = true;
  ...
  setReviewCaseInitFilter(){
    switch(this.$route.name){
      ...
      case 'REVIEW_MENU' :
        ...
        break;
      case 'REVIEW_IMPAIRMENT' :
        //聽語障
        this.setReviewCaseImpairmentFilter();
        this.isImpairmentDisable = false;
        break;
    }
  }
  ...
  //設定聽語障的固定查詢參數
  setReviewCaseImpairmentFilter(){
    // 不給選覆核類型
    this.isReviewStatusDisable = true;
    this.reviewCaseSearchForm.reviewType = [reviewTypeEnum.IMPAIRMENT];
  }
  ...
  // onImpairment(e){
  //   LoadingUtil.show();
  //   if(e.includes("IMPAIRMENT")){
  //     this.isImpairmentDisable = false;
  //   }else{
  //     this.isImpairmentDisable = true;
  //   }
  //   LoadingUtil.close();
  //   console.log("onImpairment-END");
  // }
  ...
  */
  ...
  handleInspectClick(data:ReviewCaseGrid, isReview: boolean){
    ...
    } else if (data.type == reviewTypeEnum.IMPAIRMENT){
      //聽語障
      this.isImpairFormVisible = true;
      this.reviewData.caseNo = data.caseNo;
      this.reviewData.infStep = 0;
      this.reviewData.infInfoId = data.reviewNo;
      this.reviewData.packNo = data.packNo;
  ...
  onCloseModal(){
    ...
    this.isImpairFormVisible = false
    
  }
}

function setReviewCaseImpairmentFilter() {

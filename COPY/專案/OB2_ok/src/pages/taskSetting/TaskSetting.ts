...
import { Option, TaskSettingDto, TaskSettingConditionDto } from "@fubonlife/obd-api-axios-sdk";
...
import TaskSettingConditionForm from "@/components/shared/form/taskSettingForm/TaskSettingConditionForm.vue";
...
@Component({
  components: { FblFilterBuilder, FblDataGrid ,TaskSettingForm ,TaskSettingConditionForm, HiddenFolde}
})
export default class ProductPage extends Vue {
  ...
  // form modal
  formVisible = false;
  formSubConditionVisible = false;
  formTitle = "";
  
  // misc
  masterInspected: TaskSettingDto = null;
  masterEditing: TaskSettingDto = null;
  conditionInspected: TaskSettingConditionDto = null;
  ...
  onMasterInspectClick(row: FblRow<TaskSettingDto>) {
    ...
    this.conditionInspected = row.data;
    this.openSubConditionForm();
  }

  /** 開啟電訪期限及電訪頻率設定子視窗 */
  openSubConditionForm(): void{
    this.formSubConditionVisible = true;
  }

  //------- 呼叫Condition Form Function -------
  /** 新增條件物件 */
  async doAddObj(){
    await new Promise((resolve, reject) => (this.$refs.taskSettingConditionForm as any).doAddObj());
  }
  /** 表單送出(修改/新增) */
  async doSubmit(){
    await new Promise((resolve, reject) => (this.$refs.taskSettingConditionForm as any).doSubmit());
  }
  /** 關閉Form窗 */
  doCancel(){
    this.formSubConditionVisible = false;
  }

  /** 開關TaskSettingConditionForm 並 更新列表*/
  onCoditionFormCallBack(val){
    this.formSubConditionVisible = val;
    this.reloadMaster();
  }

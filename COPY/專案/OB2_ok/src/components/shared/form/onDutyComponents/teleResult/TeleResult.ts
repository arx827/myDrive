...
import { ImpairmentReviewHistoryDto } from "@fubonlife/obd-api-axios-sdk";
...
export default class TeleResult extends Vue{
    ...
    caseClosedReason: boolean = true;
    
    //是否點選過儲存按鈕
    hasTeleResultAreaSave: boolean = false;
    impairmentCaseCloseReasonCode: TSysCommonCode[];
    ...
    public grid = {
        ...
        columns: [
            ...
            {
                dataIndex: 'callFreq',
                title: this.$t('teleResultArea_grid_callFreq').toString(), // 電訪頻率累計
                scopedSlots: { customRender: 'callFreqTemp' },
                width: 130,
            },
            {
                ...
                if(resp.data.success){
                    ...
                    this.grid.data.forEach((eachRow:any)=>{              
                        if(isCallUpResultInclude_IN_PERSON !== undefined){
                            //判斷連動前的電訪結果與連動後的電訪結果是否一致，如果不一致，將selectedTeleRes(電訪結果)清空
                            var oldContRes = eachRow.selectedContRes;
                            //有撥號資訊且有本人接聽
                            ...
                            if(oldContRes != eachRow.selectedContRes){
    ...
    readyTeleResultSave(){
        ...
        if(!validateVisitDate || this.teleResultAreaValidateForm.convenientContactEndTime.feedback || this.teleResultAreaValidateForm.convenientContactStartTime.feedback){
            return;
        }

        // 結案原因檢核 
        this.caseClosedReason = this.chkCaseClosedReason();
        if(this.caseClosedReason == true){
            //清空方便聯絡時間
            this.clearContactDate();
        }
     ...
     * 電訪結果區儲存
     */
    async toSaveTeleResult(){
        // 寫入有效的電訪頻率累計天數
        // 有效的定義如下
        // 1.聯絡結果為【聯絡上本人】或【聯絡不上】 2.此次取件有撥號歷程 3.所有撥號結果只要有一個不為【未撥出】
        let codingNoList = callUpInfoModule.callUpInfo.previousCodingNoList;
        this.$callUpApi.getCallForThisPickUsingPOST(codingNoList).then((resp)=>{
            if(resp.data){
                this.$callUpApi.getCallListForThisPickUsingPOST(codingNoList).then((resp)=>{
                    var callUpList = resp.data;
                    var isAllCallUpisNotDialer = callUpList.some((eachCullUpData)=>{
                        if(eachCullUpData.callUpResult != "NOT_DIALED"){
                            return true;
                        }else{
                            return false;
                        }
                    ...
                    console.log("電訪頻率寫檔：");
                    console.log(isAllCallUpisNotDialer);
                    ...
                    this.grid.data.forEach((eachRow:any)=>{
                        let caseNoGuid = CommonUtil.getCaseLogId(eachRow.caseNo);
                        if(isAllCallUpisNotDialer){
                            if(eachRow.selectedContRes == 'A001' || eachRow.selectedContRes == 'B001' || eachRow.selectedContRes == '聯絡上本人' || eachRow.selectedContRes == '聯絡不上'){
                                console.log("save");
                                if(caseNoGuid != ""){
                                    console.log("save-1");
                                    this.$taskSettingApi.doSaveCallLimitDetailUsingPOST(eachRow.caseNo, caseNoGuid, currentPackNo, currentPackNoGuid)
                                    .then((resp) => {
                                        console.log("doSaveCallLimitDetail success");
                                    }).catch((err) => {
                                        console.log(err);
                                        LoadingUtil.close();
                                        ErrorModalUtil.modalError("系統發生錯誤，請聯絡系統管理員處理(doSaveCallLimitDetail error)");
                                        return;
                                    })
                                }
                            }else{
                                ...
                                if(caseNoGuid != ""){
                                    console.log("del-1");
                                    this.$taskSettingApi.doDelCallLimitDetailUsingPOST(eachRow.caseNo, caseNoGuid, currentPackNo, currentPackNoGuid)
                                    .then((resp) => {
                                        console.log("doDelCallLimitDetail success");
                                    }).catch((err) => {
                                        console.log(err);
                                        LoadingUtil.close();
                                        ErrorModalUtil.modalError("系統發生錯誤，請聯絡系統管理員處理(doDelCallLimitDetail error)");
                                        return;
                                    })
                                } 
                            }
                            ...
                            if(caseNoGuid != ""){
                                ...
                                .then((resp) => {
                                    ...
                                    console.log(err);
                                    LoadingUtil.close();
                                    ErrorModalUtil.modalError("系統發生錯誤，請聯絡系統管理員處理(doDelCallLimitDetail2 error)");
                                    return;
                                })
                            } 
                        }
                    ...
                    LoadingUtil.close();
                    ErrorModalUtil.modalError("取得撥號清單資料失敗");
                })
            }
        }).catch((err)=>{
            LoadingUtil.close();
            ErrorModalUtil.modalError("寫入有效的電訪頻率累計天數失敗");
        })
        ...
        await this.$informApi.getImpairmentReviewHistoryUsingPOST(teleResultData.packNo)
            .then((res : AxiosResponse<ImpairmentReviewHistoryDto[]>) =>{ 
                this.ImpairmentReviewHistory = res.data;
        }).catch(e => {
            ErrorModalUtil.modalError("撈歷程資料:" +e.toString); // 撈歷程資料
        }).finally(() => LoadingUtil.close());


        LoadingUtil.show();
        //結案需確認是否要檢核結案原因 清單
        await this.$informApi.getImpairmentWriteBackCHKCodeUsingPOST()
                .then((resp: AxiosResponse<TSysCommonCode[]>) => {
                    this.impairmentCaseCloseReasonCode = resp.data;
                }).catch(e => {
        ...
        LoadingUtil.show();
        if(this.ImpairmentReviewHistory[0]){
            //撈歷程資料->如果有值不用檢核
            console.log("聽語障結案檢核完成");
        }else{
            //撈歷程資料->如果沒有值要確認結案原因是否在設定檔中，如果有跳檢核錯誤訊息
            this.impairmentCaseCloseReasonCode.forEach((impairmentrow) => {
                
                this.grid.data.forEach((row)=>{
                    if ( VlidationUtil.isEmpty(row.selectedCaseClosedReason) || row.selectedCaseClosedReason=="0") {
                        if(impairmentrow.codeName == row.selectedCaseClosedReason){
                            ErrorModalUtil.modalError(this.$t('Impairment_saveConfirmcancel').toString()); // 聽語障結案檢核錯誤
                        }
                    }
                })
        ...
        this.$teleResultAreaApi.saveTeleResultAreaInfoUsingPOST(teleResultData).then((resp)=>{
            ...
            if(resp.data.success){
                ...
                if((start != null && end != null) || resp.data.autoComputeVisitTime){
                    ...
                    this.caseClosedReason = this.chkCaseClosedReason();
                    //若有結案原因，則將畫面上方便連絡時段清除，且灰化不可異動
                    if(this.caseClosedReason){
                        this.isTimePickerShow = false;
                        this.teleResultAreaForm.contactDate =null;
                        this.teleResultAreaForm.contactString = "";
                        this.caseClosedReason = false;
                    }else{
                        this.isTimePickerShow = true;
                        this.caseClosedReason = true;   
                    }
                    //更新此名單已儲存方便連絡時段
                    this.updateContiuePackVisitTimeInfo(true);
                    LoadingUtil.close();
                    ...
                    //38.調整無人應答系統自動押方便聯絡時段規則
                    //B0701
                    this.$informApi.checkVisitTimeByPackNoUsingPOST(teleResultData.packNo,teleResultData)
                    .then((resp) => {
                        //若有結案原因，則將畫面上方便連絡時段清除，且灰化不可異動
                        if(this.caseClosedReason){
                            this.isTimePickerShow = false;
                            ...
                            //更新此名單已儲存方便連絡時段
                            // 判斷取回值是否為null
                            if(resp.data.visitendTime !=null && resp.data.visitstartTime !=null){
                                this.teleResultAreaForm.convenientContactStartTime = moment(resp.data.visitstartTime);
                                this.teleResultAreaForm.convenientContactStartString = resp.data.visitstartTime;
                                this.teleResultAreaForm.convenientContactEndTime = moment(resp.data.visitendTime);
                                this.teleResultAreaForm.convenientContactEndString =resp.data.visitendTime;
                                this.teleResultAreaForm.contactString = MomentUtil.transformRoc((resp.data.visitstartTime).substring(0,10).replaceAll('-','/')).substring(0,9);
                                this.teleResultAreaForm.contactDate =moment(resp.data.visitendTime);
                                this.isTimePickerShow = true;
                                this.caseClosedReason = true;
                            }
                            else{
    ...
    ************************************************************************************************************/
    ...
    onConvenientContactStartTimeChange(date, timeString, flag) {
        ...
        if(timeString.length<6){
            ...
            const oldTime = this.teleResultAreaForm.convenientContactEndString.split(':');
            // 表示 使用者點擊 時，所以 分要歸零
            if(newTime[0] !== oldTime[0]){
                timeString = `${newTime[0]}:00`;
            }else{
                timeString = `${newTime[0]}:${newTime[1]}`;
            }
            // 判斷點了 時 還是 分
            this.teleResultAreaForm.convenientContactStartTime = moment(timeString, 'HH:mm');
            this.teleResultAreaForm.convenientContactStartString = timeString;
        }else{
            // 判斷點了 時 還是 分
            this.teleResultAreaForm.convenientContactStartTime = moment(timeString, 'YYYY-MM-DDTHH:mm:ss');
            this.teleResultAreaForm.convenientContactStartString = timeString;
        }
        /**
...
chkCaseClosedReason(){
    ...
    reGetFreqData(){
        // 判斷是否符合有效定義全部重新取得相關的電訪頻率累計資料
        this.grid.data.forEach((eachRow:any)=>{
            this.$taskSettingApi.getCallLimitDataUsingPOST(eachRow.caseNo)
            .then(async (respCall:AxiosResponse<CallLimitDto>) => {
                if (respCall.data.status == 'Y'){
                    var callFreqDay;
                    var callFreqVisit;
                    var changeCallFreqDay = respCall.data.callFreqDay;
                    var changeCallFreqVisit = respCall.data.callFreqVisit; 
                    let currentPackNo = PackMatchModule.pickupResult.firstCasePack.packNo;
                    let currentPackNoGuid = PackMatchModule.pickupResult.firstCasePack.packLogNo;
                    let caseNoGuid = CommonUtil.getCaseLogId(eachRow.caseNo);

                    //先判斷當下DB是否有值
                    await this.$taskSettingApi.doChkCallLimitDetailUsingPOST(eachRow.caseNo, caseNoGuid, currentPackNo, currentPackNoGuid)
                    .then(async (resp) => {
                        let codingNoList = callUpInfoModule.callUpInfo.previousCodingNoList;
                        //DB有值時的處理，如不符合有效條件時需要-1
                        if(resp.data){
                            await this.$callUpApi.getCallListForThisPickUsingPOST(codingNoList).then(async (resp)=>{
                                var callUpList = resp.data;
                                var isAllCallUpisNotDialer = callUpList.some((eachCullUpData)=>{
                                    if(eachCullUpData.callUpResult != "NOT_DIALED"){
                                        return true;
                                ...
                                if(!isAllCallUpisNotDialer){
                                    let nowFreqDay = await this.$taskSettingApi.getAddOrNotFreqUsingPOST(eachRow.caseNo);
                                    if(nowFreqDay.data == 1){
                                        changeCallFreqDay = String(Number(respCall.data.callFreqDay) - 1);
                                    }
                                    changeCallFreqVisit = String(Number(respCall.data.callFreqVisit) - 1);
                                }else{
                                    if(!(eachRow.selectedContRes == 'A001' || eachRow.selectedContRes == 'B001' || eachRow.selectedContRes == '聯絡上本人' || eachRow.selectedContRes == '聯絡不上')){
                                        let nowFreqDay = await this.$taskSettingApi.getAddOrNotFreqUsingPOST(eachRow.caseNo);
                                        if(nowFreqDay.data == 1){
                                ...
                                console.log("DB have data："+err);
                            })
                        }else{ //DB沒值時的處理，判斷是否符合有效條件決定是否要+1
                            await this.$callUpApi.getCallListForThisPickUsingPOST(codingNoList).then(async (resp)=>{
                                var callUpList = resp.data;
                                var isAllCallUpisNotDialer = callUpList.some((eachCullUpData)=>{
                                    if(eachCullUpData.callUpResult != "NOT_DIALED"){
                                ...
                                console.log("DB no data："+err);
                            })
                        }
                    }).catch((err) => {
                        console.log("計算頁面上的電訪頻率失敗(doChk)");
                    })
                    if(Number(changeCallFreqDay) < Number(respCall.data.callFreqDayDefault)){
                        callFreqDay = "<font color='red'>"+changeCallFreqDay+"</font>";
                    }else{
                        callFreqDay = changeCallFreqDay;
                    }
                    if(Number(changeCallFreqVisit) < Number(respCall.data.callFreqVisitDefault)){
                        callFreqVisit = "<font color='red'>"+changeCallFreqVisit+"</font>";
                    }else{
                        callFreqVisit = changeCallFreqVisit;
                    }
                    eachRow.callFreq = callFreqDay + "天" + callFreqVisit + "訪";
                } else {
                    eachRow.callFreq = "NA";
                }  
            }).catch((err) => {

...
export default class RiskControl extends Vue {
    ...
    // 內容有任何異動時，應該更新相關呈現資料
    @Watch('theRiskControlDetailsData', { deep: true })
    watchRiskControlDetailForm() {
        this.restData();
    }
    ...
    created() {
        ...
        // 【單件年化保費金額】【應繳保費】【每期保險費】需轉化為千分位格式
        this.theRiskControlDetailsData.policySingleAnnualisedPremiums = this.numberToCurrencyNo(this.theRiskControlDetailsData.policySingleAnnualisedPremiums);
        this.theRiskControlDetailsData.policyPolicyCost = this.numberToCurrencyNo(this.theRiskControlDetailsData.policyPolicyCost);
        this.theRiskControlDetailsData.policyPrePolicyCost = this.numberToCurrencyNo(this.theRiskControlDetailsData.policyPrePolicyCost);
        // 要保人指定匯款帳戶
        this.applicantDesignatedRemittanceAccountData.data = this.theRiskControlDetailsData.payBeneAccounts;
        // 變更後受益人清單
        this.listOfBeneficiariesAfterChangeData.data = this.theRiskControlDetailsData.benefitBeneInfos;

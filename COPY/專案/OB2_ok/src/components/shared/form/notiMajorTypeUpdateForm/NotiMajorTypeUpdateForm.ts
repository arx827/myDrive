...
export default class NotiMajorTypeUpdateaForm extends Vue {
  ...
  reset() {
    this.notiMajorTypeUpdateForm = {
      userMajorTypeId: this.initData.userMajorTypeId,
  ...
  submit() {
      ...
      let notiUserMajorTypeIdValidation: boolean = !this.notiMajorTypeValidationForm.userMajorTypeId.feedback? true : false;
      if(notiDescriptionValidation && notiUserMajorTypeIdValidation){
      if (this.isEditing) {//編輯模式
        Modal.confirm({
          ...
          onOk: () => {
            ...
            updateBody.userMajorTypeId = this.notiMajorTypeUpdateForm.description;
        ...
        Modal.confirm({
          ...
          onOk: () => {
            ...
            createBody.reviewUserTypeId = this.notiMajorTypeUpdateForm.userMajorTypeId;
            ...
            this.$notifyBasicSettingApi.insertNotiTypeUsingPOST(createBody)
              .then((resp: AxiosResponse<ReviewType>) => {
                if(resp.data.reviewUserTypeId == "repeat"){
                  ErrorModalUtil.modalError(this.$t("notification_majorType_userMajorTypeId_repeat").toString());
                }else{
  ...
  notiMajorTypeUpdateFormRules: { [key: string]: ValidationRule[] } = {
    ...
    userMajorTypeId: [{ validator: this.validateUserMajorTypeId, trigger: "blur" }],
  };

  //結案表單驗證物件
  notiMajorTypeValidationForm:  NotiMajorTypeValidateForm = {
    description: { feedback: false, hoverVisible: false, msg: "" },
    userMajorTypeId: { feedback: false, hoverVisible: false, msg: "" },

  }
  // //顯示popOver的flag
  isDescriptionVisible: boolean = false;
  isUserMajorTypeIdVisible: boolean = false;
  ...
  validateUserMajorTypeId(rule, value, callback){
    CommonUtil.feildValidateWithVisible(this.notiMajorTypeValidationForm.userMajorTypeId, false, "", false);

    if (ValidationUtil.isEmpty(value)) {
      CommonUtil.feildValidateWithVisible(this.notiMajorTypeValidationForm.userMajorTypeId, true, this.$t("notification_majorType_userMajorTypeId_not_blank").toString(), false);
      callback(false);
      return;
    }

    if (!ValidationUtil.accountValidation(value)) {
      CommonUtil.feildValidateWithVisible(this.notiMajorTypeValidationForm.userMajorTypeId, true, this.$t("notification_majorType_userMajorTypeId_number").toString(), false);
      callback(false);
      return;
    }

    if (value.length != 2) {
      CommonUtil.feildValidateWithVisible(this.notiMajorTypeValidationForm.userMajorTypeId, true, this.$t("notification_majorType_userMajorTypeId_length").toString(), false);
      callback(false);
      return;
    }
  ...
  notiUserMajorTypeIdMouseOver() {
    if (this.notiMajorTypeValidationForm.userMajorTypeId.feedback) {
      this.isUserMajorTypeIdVisible = true;
    } else {
      this.isUserMajorTypeIdVisible = false;
    }

  }

  onUserMajorTypeIdChange() {
      this.validateDescription(null, this.notiMajorTypeUpdateForm.userMajorTypeId, () => { });
  }

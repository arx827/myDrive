   ...
   */
   onSendNotification() {
    ...
    if(submit){
      ...
      let packNo = this.propPackNo;
      if(VlidationUtil.isEmpty(packNo)){
        packNo = PackMatchModule.pickupResult.firstCasePack.packNo;
      }
      let packLogNo = this.packLogId;
      if(VlidationUtil.isEmpty(packLogNo)){
        packLogNo = PackMatchModule.pickupResult.firstCasePack.packLogNo;
      }
      LoadingUtil.show();

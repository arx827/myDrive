...
import { AutoCheckByCallResult, NotificationCallUpAgentContactPersonForm } from "./model";
...
export default class NotificationCallUpAgentContactPerson extends Vue {
    ...
    created() {
        ...
        this.initContactAgentPersonData();
    }

    /**
     * 初始化聯絡對象的映射資料
     */
    initContactAgentPersonData() {
        LoadingUtil.show();
    ...
    watchSearchOtherAgent(newVal, oldVal) {
        if (!VlidationUtil.isEmpty(newVal)) {
            if (this.agentPickValue == 0) {
                ...
                // 整理聯絡對象 Grid Data 資料
                this.pushAgentGridData(this.notiCallUpAgentContactPersonForm);
            }
        }
    ...
    watchAgentPick(newVal: number, oldVal: number) {
        if (!VlidationUtil.isEmpty(newVal)) {
            ...
            // 由於使用者選擇完聯絡對象資料時，那些下拉選單都會更改，故在此需要初始化，才能映射到所有的值
            this.initContactAgentPersonData();
            // 整理聯絡對象 Grid Data 資料
            this.pushAgentGridData(this.notiCallUpAgentContactPersonForm);
        }
     ...
     * 本次撥號結果下拉選單變化
     */
    onCallUpResultChange(data) {
        data.contactResult = "";
        data.contactDetail = "";

        // 當撥號結果選到【無人應答、電話忙線、語音信箱】，訊息發送會預勾選並出現提示訊息『請確認是否需發送訊息給業務員』
        this.isNotContact = Object.values(AutoCheckByCallResult).includes(data.callUpResult);
        this.$emit("isSendMessage", this.isNotContact);
    }
     ...
     * @param data 
     */
    onExtensionDialClick(data) {
        // 撥分機前驗證
        let validation = true;
        data.extension = data.extension ? data.extension.trim() : data.extension;
        if (VlidationUtil.isEmpty(data.extension)) {
            ErrorModalUtil.modalError(this.$t('callUpF_extensionIsEmpty').toString()); // 分機 欄位不可為空
            validation = false;
            return;
        } else if (!VlidationUtil.numberValidation(data.extension)) {
     ...
     * @param data 
     */
    pushAgentGridData(data: NotificationCallUpAgentContactPersonForm) {
        // 取得暫存的撥號資訊
     ...
     * @param data 
     */
    innitAgentData(data: NotificationCallUpAgentContactPersonForm) {
        this.gridData.data = [];
        this.gridData.data.push({
            seqNo: this.getGridRowKey(),
            phoneNo: data.agentMob,
            isProp: true,
            isDataAdd: false,
            isCallup: false,
            isResult: false,
        ...
        if (!VlidationUtil.isEmpty(data.agentOfficeTel)) {
            this.gridData.data.push({
                seqNo: this.getGridRowKey(),
                phoneNo: data.agentOfficeTel,
                extension: data.agentOfficeTelExt,
                isProp: true,
                isDataAdd: false,
                isCallup: false,
                isResult: false,

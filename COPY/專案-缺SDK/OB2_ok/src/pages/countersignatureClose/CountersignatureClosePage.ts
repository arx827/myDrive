...
export default class CountersignaturePage extends Vue {
  ...
  infSearchRule: { [key: string]: ValidationRule[] } = {
    ...
    replyDateStart: [{ validator: this.validateReplyDateStart, trigger: "blur" }],
    replyDateEnd: [{ validator: this.validateReplyDateEnd, trigger: "blur" }],
  };
  ...
  infSearchValidateForm: InfSearchValidateForm = {
    ...
    replyDateStart: { hover: "", feedback: false, state: "success", msg: "" },
    replyDateEnd: { hover: "", feedback: false, state: "success", msg: "" },
  ...
  isReplyDateStartVisible: boolean = false;
  isReplyDateEndVisible: boolean = false;
  ...
  // ===================================== 會辦回覆日期 DatePicker 事件 B1530 2022/04/06 ========================================================

  //起始日期變動
  onReplyDateStartChange(date) {
    this.infSearchForm.replyStartDateString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(date)));
    this.isReplyDateStartVisible = false;
    this.isReplyDateEndVisible = false;
    this.feildValidate(this.infSearchValidateForm.replyDateStart, false, "success", "", "");
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
    this.replyDateVaildate();
    
  }

  //結束日期變動
  onReplyDateEndChange(date) {
    this.infSearchForm.replyEndDateString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(date)));
    this.isReplyDateStartVisible = false;
    this.isReplyDateEndVisible = false;
    this.feildValidate(this.infSearchValidateForm.replyDateStart, false, "success", "", "");
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
  ...
  //滑鼠移入
  eventMouseOverReplyDateStart() {
    if (this.infSearchValidateForm.replyDateStart.feedback) {
      this.isReplyDateStartVisible = true;
    } else {
      this.isReplyDateStartVisible = false;
    }
  }
  ...
  //滑鼠移入
  eventMouseOverReplyDateEnd() {
    if (this.infSearchValidateForm.replyDateEnd.feedback) {
      this.isReplyDateEndVisible = true;
    } else {
      this.isReplyDateEndVisible = false;
    }
  }
  ...
  clearReplyStartDate() {
    ...
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
    this.isReplyDateStartVisible = false;
    this.infSearchForm.replyStartDateString = "";
    this.infSearchForm.replyStartDate = null;
    this.replyDateVaildate();
  }
  ...
  clearReplyEndDate() {
    ...
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
    this.isReplyDateEndVisible = false;
    this.infSearchForm.replyEndDateString = "";
    this.infSearchForm.replyEndDate = null;
    this.replyDateVaildate();
  }
  ...
  checkManualInputReplyStartDate(data: any) {
    ...
    if (parseDate) {
      ...
      this.isReplyDateStartVisible = true;
      // 日期錯誤
      this.feildValidate(this.infSearchValidateForm.replyDateStart, true, "error", "hover", this.$t('global_dateError').toString());
    }
    this.infSearchForm.replyStartDate = parseDate ? parseDate : this.infSearchForm.replyStartDate;
    this.infSearchForm.replyStartDateString = parseDate ?
      MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(this.infSearchForm.replyStartDate.toString()))) :
      data.currentTarget.value.match(/\d+\/\d+\/\d+/)[0];
  }

  //手動輸入結束日期
  checkManualInputReplytEndDate(data: any) {
    this.isReplyDateStartVisible = false;
    this.isReplyDateEndVisible = false;
    this.feildValidate(this.infSearchValidateForm.replyDateStart, false, "success", "", "");
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
    ...
    if (parseDate) {
      ...
      this.isReplyDateEndVisible = true;
      // 日期錯誤
      this.feildValidate(this.infSearchValidateForm.replyDateEnd, true, "error", "hover", this.$t('global_dateError').toString());
    }
    this.infSearchForm.replyEndDate = parseDate ? parseDate : this.infSearchForm.replyEndDate;
    this.infSearchForm.replyEndDateString = parseDate ?
      MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(this.infSearchForm.replyEndDate.toString()))) :
      data.currentTarget.value.match(/\d+\/\d+\/\d+/)[0];
  }

  // ===================================== 會辦回覆日期 DatePicker 事件 END ========================================================
  ...
  setInfInitFilter() {
    ...
    this.infSearchForm.caseClosedStatus='W';      
  }
  // ===================================== Search 事件 ========================================================
  //將結案狀態設為無
  onClickSearchButton(){
    this.infSearchForm.caseClosedStatus='';
    this.infSearch();
  }
  ...
  infSearch() {
    if (this.validateSearch()) {
      ...
      const replyDateStart = FiltersUtil.setFilterParam("replyDate", FblOperator.GEQ, moment(this.infSearchForm.replyStartDate).startOf("day").toISOString(true));
      const replyDateEnd = FiltersUtil.setFilterParam("replyDate", FblOperator.LEQ, moment(this.infSearchForm.replyEndDate).endOf("day").toISOString(true));
      ...
      const caseCloseStatus=FiltersUtil.setFilterParam("caseClosedStatus",FblOperator.EQ,this.infSearchForm.caseClosedStatus);
      this.infFilter = FiltersUtil.setFilters(custId, sendStart, sendEnd, custName, expireStart, expireEnd, replyDateStart, replyDateEnd, infInfoId, infTypeId,caseCloseStatus);
  ...
  exportSearchResult() {
    if (this.validateSearch()) {
      ...
      LoadingUtil.show();
      this.otherFilter.filter = this.infFilter;
      this.$informApi.exportReplyInformSearchUsingPOST(this.otherFilter, { responseType: 'blob' })
      .then((resp)=>{
        LoadingUtil.close();
        this.dealDownLoadData(resp.data, "會辦查詢" + ".xlsx"); 
        MessageUtil.messageInfo(this.$t('global_exportSuccess').toString()); //匯出成功
      }).catch((err)=>{
        // 會辦匯出失敗
        LoadingUtil.close();
        ErrorModalUtil.modalError(this.$t('global_exportFailure').toString())
      })  
  ...
  replyDateVaildate() {
    let validate = true;
    this.validateReplyDateStart(null, this.infSearchForm.replyStartDateString, () => {
      if (this.infSearchValidateForm.replyDateStart.state == 'error') {
        validate = false;
      }
    })
    this.validateReplyDateEnd(null, this.infSearchForm.replyEndDateString, () => {
      if (this.infSearchValidateForm.replyDateEnd.state == 'error') {
        validate = false;
      }
    });
    //起始與結束皆符合規範才進一步判斷起訖區間
    if (validate && !ValidationUtil.isEmpty(this.infSearchForm.replyStartDateString) && !ValidationUtil.isEmpty(this.infSearchForm.replyEndDateString)) {
      this.validateReplyDateStartAndEndDate(null, this.infSearchForm.replyStartDate, this.infSearchForm.replyEndDate, this.infSearchForm.replyStartDateString, this.infSearchForm.replyEndDateString, this.isReplyDateStartVisible, this.isReplyDateStartVisible, () => {
        if (this.infSearchValidateForm.replyDateStart.state == 'error' || this.infSearchValidateForm.replyDateEnd.state == 'error') {
          validate = false;
        }
      })
    }
  ...
  checkFilterCount() {
    ...
    Object.keys(this.infSearchForm).forEach((key => {
      if (key != 'sendStartDate' && key != 'sendEndDate' && key != 'expiryStartDate' && key != 'expiryEndDate'&&key!='replyStartDate'&&key!='replyEndDate' ) {
    ...
    if (!isAbleToSearch) {
      ...
      if (filterColumn.some((c) => c == "infType")) {
        count++;
        filterColumn = filterColumn.filter((c) => c != "infType");
      }
  ...
  * @returns 
  */
  validateReplyDateStart(rule, value, callback) {
    this.feildValidate(this.infSearchValidateForm.replyDateStart, false, "success", "", "");
    if (!ValidationUtil.isEmpty(value)) {
      const parseDate = this.formatter.parse(value);
      if (parseDate) {
        ...
        this.isExpiryStartVisible = true;
        this.feildValidate(this.infSearchValidateForm.replyDateStart, true, "error", "hover", this.$t('global_dateError').toString()); //日期錯誤
        callback(() => { });
      }
    }
    callback();
 ...
 * @returns 
 */
  validateReplyDateEnd(rule, value, callback) {
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
    if (!ValidationUtil.isEmpty(value)) {
      const parseDate = this.formatter.parse(value);
      if (parseDate) {
        ...
        this.isExpiryEndVisible = true;
        this.feildValidate(this.infSearchValidateForm.replyDateEnd, true, "error", "hover", this.$t('global_dateError').toString()); //日期錯誤
        callback(() => { });
      }
    }
    callback();
...
* @returns 
*/
  validateReplyDateStartAndEndDate(rule, startDate, endDate, startString, endString, dateDisableStart, dateDisableEnd, callback) {
    this.feildValidate(this.infSearchValidateForm.replyDateStart, false, "success", "", "");
    this.feildValidate(this.infSearchValidateForm.replyDateEnd, false, "success", "", "");
    if (startString == endString || moment(startDate).isBefore(endDate)) {
      dateDisableStart = false;
      ...
      dateDisableStart = true;
      dateDisableEnd = true;
      if (!ValidationUtil.isEmpty(this.infSearchForm.replyStartDateString) && !ValidationUtil.isEmpty(this.infSearchForm.replyEndDateString)) {
        this.feildValidate(this.infSearchValidateForm.replyDateStart, true, "error", "hover", this.$t('global_pleaseInputCorrectStartAndEndDate').toString()); //請輸入正確的起訖日期
        this.feildValidate(this.infSearchValidateForm.replyDateEnd, true, "error", "hover", this.$t('global_pleaseInputCorrectStartAndEndDate').toString()); //請輸入正確的起訖日期
        callback(() => { });
      }
    }
    callback();

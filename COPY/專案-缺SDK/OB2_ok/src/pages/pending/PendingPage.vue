<template>
  <div tabindex="-1" @keyup.enter="peddingSearch">
        ...
        >
          ...
          <a-row :gutter="24" style="margin-left: 30px">
            ...
            <a-col :span="4">
              ...
              <a-form-model-item
                class="dateStart"
                :label="$t('pedding_caseCloseDate')"
                prop="caseCloseStartDate"
                style="margin-bottom: 0px"
                :has-feedback="callCommonUtilFeild(peddingSearchValidateForm.caseCloseStartDate).feedback"
                :validateStatus="callCommonUtilFeild(peddingSearchValidateForm.caseCloseStartDate).state"
              >
                <a-popover
                  placement="top"
                  :content="callCommonUtilFeild(peddingSearchValidateForm.caseCloseStartDate).msg"
                  :trigger="callCommonUtilFeild(peddingSearchValidateForm.caseCloseStartDate).hover"
                  :visible="callCommonUtilFeild(peddingSearchValidateForm.caseCloseStartDate).hoverVisible"
                  @visibleChange="callCommonUtilFeildVisibleChange(peddingSearchValidateForm.caseCloseStartDate)"
                  :destroyTooltipOnHide="true"
                >
                  <DatePicker
                    :formatter="formatter"
                    @change="onCaseCloseStartDateChange"
                    v-model="peddingSearchForm.datePickerCaseCloseStartDate"
                    @clear="clearCaseCloseDate('StartDate')"
                    style="width: 100%"
                  >
                    <!-- 請選擇日期(起) -->
                    <a-input
                      slot="input"
                      @pressEnter="checkManualInputCaseCloseStartDate"
                      :value="peddingSearchForm.caseCloseStartDateString"
                    ></a-input>
                    <i
                      v-if="callCommonUtilFeild(peddingSearchValidateForm.caseCloseStartDate).feedback"
                      slot="icon-calendar"
                    ></i>
                    <!-- 如果檢核有誤置換原本的叉叉icon成另一個，並設成白色避免兩個icon重疊 -->
                    <i
            ...
            <a-col :span="3">
              ...
              <a-form-model-item
                class="dateEnd"
                label="~"
                prop="caseCloseEndDate"
                style="margin-bottom: 0px"
                :has-feedback="callCommonUtilFeild(peddingSearchValidateForm.caseCloseEndDate).feedback"
                :validateStatus="callCommonUtilFeild(peddingSearchValidateForm.caseCloseEndDate).state"
              >
                <a-popover
                  placement="top"
                  :content="callCommonUtilFeild(peddingSearchValidateForm.caseCloseEndDate).msg"
                  :trigger="callCommonUtilFeild(peddingSearchValidateForm.caseCloseEndDate).hover"
                  :visible="callCommonUtilFeild(peddingSearchValidateForm.caseCloseEndDate).hoverVisible"
                  @visibleChange="callCommonUtilFeildVisibleChange(peddingSearchValidateForm.caseCloseEndDate)"
                  :destroyTooltipOnHide="true"
                >
                  <DatePicker
                    :formatter="formatter"
                    @change="onCaseCloseEndDateChange"
                    v-model="peddingSearchForm.datePickerCaseCloseEndDate"
                    @clear="clearCaseCloseDate('EndDate')"
                    style="width: 100%"
                  >
                    <!-- 請選擇日期(訖) -->
                    <a-input
                      slot="input"
                      @pressEnter="checkManualInputCaseCloseEndDate"
                      :value="peddingSearchForm.caseCloseEndDateString"
                    ></a-input>
                    <i
                      v-if="callCommonUtilFeild(peddingSearchValidateForm.caseCloseEndDate).feedback"
                      slot="icon-calendar"
                    ></i>
                    <!-- 如果檢核有誤置換原本的叉叉icon成另一個，並設成白色避免兩個icon重疊 -->
                    <i
    ...
    <a-row :gutter="[23, 23]">
      <a-col :span="24">
        <FblDataGrid
          ...
          @handleCheckAllSelection="handleCheckAllSelection"
          style="padding-left: 24px; padding-right: 12px"
          ref="peddingGrid"
        >
        <template v-slot:questName="slotProps">
          <a v-if="slotProps.data.questName==''" @click="questPreview(slotProps.data.packNo,slotProps.data.caseNo)">
            {{$t('pedding_page_grid_quest_preview')}}
          </a>
          <a v-else-if="slotProps.data.questName==null" @click="questPreview(slotProps.data.packNo,slotProps.data.caseNo)">
            {{$t('pedding_page_grid_quest_preview')}}
          </a>
          <a v-else @click="questPreview(slotProps.data.packNo,slotProps.data.caseNo)">{{slotProps.data.questName}}</a>
        </template>
        </FblDataGrid>

    <!--問卷預覽Modal-START------------------------------------------------>
    <a-modal
        class="ShowQuestPreviewModal"
        :visible="idShowQuestPreviewModal"
        :title="$t('pedding_page_grid_quest_preview')"
        :closable="true"
        :destroyOnClose="true"
        :keyboard="true"
        :mask="false"
    ...
    <!--自訂按鈕區-->
    <template #footer>
        <a-button key="OK" @click="onCloseModal()">關閉</a-button>
    </template>
      <div class="opencheckIdDiv">
        <!-- 問卷「開場白」顯示區塊 -->
      ...
      <div class="questModal" id="questModal">
        <!-- 問卷「題目」顯示區塊 -->
        <div v-if="true" class="sectionRight-block">
          <BlockHeader
            ref="headerQuest"

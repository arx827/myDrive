<template>
  <div class="onDutyPage">
      ...
      <div class="top-feature">
        <template v-for="item in topFeature">
          <div
            ...
            v-if="((item.key != 'appeal' && item.key != 'impairment') || isCustMark) || (item.key == 'impairment' && showImpairmentMark)"
            
            @click="topRightButtonAction(item.key)"
          >
            <template v-if="!['alert','impairment'].includes(item.key)"> {{ item.label }} </template>
            <template v-if="item.key == 'alert'"> {{ item.label }} 
              <a-icon
            ...
            <template v-if="item.key == 'impairment'" >
             {{ item.label }} 
              <a-icon
                v-if="true"
                type="exclamation-circle"
    ...
    <div class="odp-content-body">
      ...
      >
        <RiskControl
          ...
          :tobdCasePolicyHearingData="tobdCasePolicyHearingData"
        />
        <template slot="footer">
          <a-button
            type="primary"
    ...
    <!-- 聽語障Modal -->
    <a-modal
      v-model="isImpairmentFormVisible"
      title="聽語障"
      width="65%"
      :closable="true"
      :maskClosable="false"
      ...
      :destroyOnClose="true"
    >
      <ImpairmentForm @onLeave="onCloseModal('isImpairmentFormVisible')" />
    </a-modal>
    ...
    >
      <NotiInfNotCloseRemind
        :propNotiInfNotCloseList="notiInfNotCloseList"

...
import { ComponentDto, GetRecordDto, GetRecordUrlByInumResponse, RecordPaginateReqDto, ResponseEntity, TobdRecordHistoryDto, TobdRecordHistoryDtoActionEnum } from "@fubonlife/obd-api-axios-sdk";
...
export default class GetRecordPage extends Vue {
  // 畫面元件權限控管
  authComponent: AuthComonent = {
    COM_RECORD_RESULT_SEARCH: {
      show: true,
      enable: false
    },
    COM_RECORD_RESULT_DOWNLOAD: {
      show: true,
      enable: false
    },
    COM_RECORD_RESULT_EXPORT: {
      show: true,
      enable: false
    },
  };
  ...
  recordPageSearchForm: RecordPageSearchForm = {
    dialStart: moment(new Date()).startOf("day"), // 錄音時間區間(開始)
    dialEnd: moment(new Date()).startOf("day"), // 錄音時間區間(結束)
    dialStartString: MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date())), // 錄音時間區間(開始)字串
    dialEndString: MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date())), // 錄音時間區間(結束)字串
    datePickerDialStart: moment(new Date()).startOf("day").toDate(), // 錄音時間區間(結束)日期
    datePickerDialEnd: moment(new Date()).startOf("day").toDate(), // 錄音時間區間(結束)日期
    userId: "", // 電訪員帳號
    extNo: "", // 電話號碼(來電)
    telNo: "", // 電話號碼(外撥)
    casePolicy: "", // 保單號碼(全)
  ...
  // 錄音調檔查詢結果顯示設定
  grid: FblPDataGridHolder<GetRecordDto> = {
    rowKey: "rowKey",
    data: [],
    pagination: {
    ...
    columns: [
      {
        ...
        template: "playTemp",
        title: this.$t('getRecordPage_play').toString(), // 播放
        align: 'center',
      ...
      },
      {
        ...
        title: this.$t('getRecordPage_download').toString(), // 下載
        align: 'center',
        width: CommonUtil.countColumnWidth(2),
      },
      {
        ...
        property: "dialStart",
        title: this.$t('getRecordPage_dialStart').toString(), // 錄音開始時間
        align: 'center',
      ...
      },
      {
        type: FblColumnType.PLAIN,
        property: "listNo",
        title: this.$t('getRecordPage_listNo').toString(), // 名單序號
        align: 'center',
        width: CommonUtil.countColumnWidth(4),
      },
      {
      ...
      },
      {
        type: FblColumnType.PLAIN,
        property: "seconds",
        title: this.$t('getRecordPage_seconds').toString(), // 錄音(秒)
        align: 'center',
        width: CommonUtil.countColumnWidth(4),
      },
      {
        type: FblColumnType.PLAIN,
        property: "dialEnd",
        title: this.$t('getRecordPage_dialEnd').toString(), // 錄音結束時間
        align: 'center',
        width: CommonUtil.countColumnWidth(6),
      },
    ]
    ...
    this.$authApi.getAuthComponentUsingGET(this.$route.path)
      .then((res: AxiosResponse<ComponentDto>) => {
        if (res.data.component) {
          this.authComponent.COM_RECORD_RESULT_DOWNLOAD = ValidationUtil.isEmpty(res.data.component.COM_RECORD_RESULT_DOWNLOAD) ?
            this.authComponent.COM_RECORD_RESULT_DOWNLOAD : res.data.component.COM_RECORD_RESULT_DOWNLOAD;
          this.authComponent.COM_RECORD_RESULT_EXPORT = ValidationUtil.isEmpty(res.data.component.COM_RECORD_RESULT_EXPORT) ?
            this.authComponent.COM_RECORD_RESULT_EXPORT : res.data.component.COM_RECORD_RESULT_EXPORT;
          this.authComponent.COM_RECORD_RESULT_SEARCH = ValidationUtil.isEmpty(res.data.component.COM_RECORD_RESULT_SEARCH) ?
            this.authComponent.COM_RECORD_RESULT_SEARCH : res.data.component.COM_RECORD_RESULT_SEARCH;
        }
      }).catch((err) => {
        console.log(err);
  ...
  dialStartValidator(rule, value, callback) {
    ...
    if (!ValidationUtil.isEmpty(value)) {
      ...
      this.isDialStartVisible = true;
      CommonUtil.feildValidateWithVisible(this.recordPageSearchValidateForm.dialStart, true, this.$t('getRecordPage_dialStartRequired').toString(), false); // 錄音時間區間(開始) 必填
      callback(() => { });
    }
    callback();
  }
  ...
  dialEndValidator(rule, value, callback) {
    ...
    if (!ValidationUtil.isEmpty(value)) {
      ...
      this.isDialEndVisible = true;
      CommonUtil.feildValidateWithVisible(this.recordPageSearchValidateForm.dialEnd, true, this.$t('getRecordPage_dialEndRequired').toString(), false); // 錄音時間區間(結束) 必填
      callback(() => { });
    }
    callback();
  }
  ...
  */
  ...
  userIdValidator(rule, value, callback) {
    ...
    if (!ValidationUtil.isEmpty(this.recordPageSearchForm.userId)) {
      if (ValidationUtil.alphabetAndNumberValidation(this.recordPageSearchForm.userId)) {
        ...
        CommonUtil.feildValidateWithVisible(this.recordPageSearchValidateForm.userId, true, this.$t('getRecordPage_userIdLetterNumberOnly').toString(), false); // 電訪員帳號 僅可輸入英文與數字
        callback(() => { });
      }
    }
    callback();
  ...
  searchGetRecordPage() {
    ...
    if (this.validateSearch(true)) {
      ...
      if ((VlidationUtil.isEmpty(this.recordPageSearchForm.dialStartString) || VlidationUtil.isEmpty(this.recordPageSearchForm.dialStartString)) &&
        ...
        this.recordPageSearchForm.dialStart = moment(new Date()).startOf("day"); // 錄音時間區間(開始)
        this.recordPageSearchForm.dialEnd = moment(new Date()).startOf("day"); // 錄音時間區間(結束)
        this.recordPageSearchForm.dialStartString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date())); // 錄音時間區間(開始)字串
        this.recordPageSearchForm.dialEndString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date())); // 錄音時間區間(結束)字串
        this.recordPageSearchForm.datePickerDialStart = moment(new Date()).startOf("day").toDate(); // 錄音時間區間(結束)日期
        this.recordPageSearchForm.datePickerDialEnd = moment(new Date()).startOf("day").toDate(); // 錄音時間區間(結束)日期
      }
      ...
      const dialStart = FiltersUtil.setFilterParam("dialStart", FblOperator.GEQ, dialStartIsoString);
      const dialEnd = FiltersUtil.setFilterParam("dialEnd", FblOperator.LEQ, dialEndIsoString);
      const userId = FiltersUtil.setFilterParam("userId", FblOperator.EQ, this.recordPageSearchForm.userId);
      ...
      const policyNo = FiltersUtil.setFilterParam("policyNo", FblOperator.EQ, this.recordPageSearchForm.policyNo);
      const policySeq = FiltersUtil.setFilterParam("policySeq", FblOperator.EQ, this.recordPageSearchForm.policySeq);
      const policyIdDup = FiltersUtil.setFilterParam("policyIdDup", FblOperator.EQ, this.recordPageSearchForm.policyIdDup);
      this.recordSearchFilters = FiltersUtil.setFilters(dialStart, dialEnd, userId, extNo, telNo, policyNo, policySeq, policyIdDup);
  ...
  reload() {
    if (this.searchFlag) {
      ...
      LoadingUtil.show();
      this.$recordApi.paginateUsingPOST(({
        page: this.grid.pagination.current - 1,
        size: this.grid.pagination.pageSize,
        vwObRecDialerFilter: this.recordSearchFilters,
        vwObRecCampFilter: this.recordSearchFilters,
        recordViewFilter: this.recordSearchFilters,
      } as RecordPaginateReqDto))
        .then((resp) => {
  ...
  exportSearchResult() {
    if (!this.isExportDisable) {
      if (this.grid.data.length == 0) {
        ...
        LoadingUtil.show();
        this.$recordApi.exportRecordResultUsingPOST(({
          page: this.grid.pagination.current - 1,
          size: this.grid.pagination.pageSize,
          vwObRecDialerFilter: this.recordSearchFilters,
          vwObRecCampFilter: this.recordSearchFilters,
          recordViewFilter: this.recordSearchFilters,
        } as RecordPaginateReqDto), { responseType: 'blob' })
          .then((resp) => {
            this.dealDownLoadData(resp.data, this.$t('getRecordPage_recordsearchResultExport').toString()); // 錄音調檔.xlsx
  ...
  */
   ...
   * @param data 
   */
  displayRecord(data: GetRecordDto) {
    this.downloadPlayLogging(data, TobdRecordHistoryDtoActionEnum.P);
    console.log("inumDto", data.inumDto);
    if (!ValidationUtil.isEmpty(data.inumDto.inum)) {
      this.openInumRecordUrl(data.inumDto.inum)
    } else if (!ValidationUtil.isEmpty(data.inumDto.eduid) && !ValidationUtil.isEmpty(data.inumDto.recordUrl)) {
      this.openEduidRecordUrl(data.inumDto.recordUrl);
    }
  }
   ...
   * @param data 
   */
  downloadRecord(data: GetRecordDto) {
    this.downloadPlayLogging(data, TobdRecordHistoryDtoActionEnum.D);
    console.log("inumDto", data.inumDto);
    LoadingUtil.show();
    this.$recordApi.downloadRecordUsingPOST(data.inumDto, { responseType: 'blob' })
      .then((resp: AxiosResponse<ResponseEntity>) => {
        console.log("resp.data", resp.data);
        let fileName = ".wav";
        if (!ValidationUtil.isEmpty(data.inumDto.recordUrl)) {
          this.dealDownLoadData(resp.data,
            data.inumDto.recordUrl.substring(data.inumDto.recordUrl.lastIndexOf('/') + 1));
        } else if (!ValidationUtil.isEmpty(data.inumDto.inum)) {
          this.dealDownLoadData(resp.data, [data.inumDto.inum, fileName].join());
        } else {
          this.dealDownLoadData(resp.data, this.$t('getRecordPage_recordFileName').toString());
        }
        MessageUtil.messageSuccess(this.$t('getRecordPage_downloadSuccess').toString()); // 下載成功
      }).catch((err) => {
        console.error(err);
        ErrorModalUtil.modalError(this.$t('getRecordPage_downloadFailure').toString()); // 下載失敗
      }).finally(() => { LoadingUtil.close(); });
  }
   ...
   * @param iNum 
   */
  openInumRecordUrl(iNum: string) {
    this.isLoading = true;
    this.showRecordPlayer = false;
    this.$historyApi.getRecordUrlByInumUsingPOST(iNum).then((resp: AxiosResponse<GetRecordUrlByInumResponse>) => {
      if (resp.data.apiStatus && !ValidationUtil.isEmpty(resp.data.recordUrl)) {
        this.recordPlayUrl = resp.data.recordUrl;
        console.log("this.recordPlayUrl", this.recordPlayUrl);
   ...
   * @param recordUrl 
   */
  openEduidRecordUrl(recordUrl: string) {
    this.recordPlayUrl = recordUrl;
    console.log("this.recordPlayUrl", this.recordPlayUrl);
    this.showRecordPlayer = true;
  }
   ...
   * @param action 
   */
  downloadPlayLogging(data: GetRecordDto, action: TobdRecordHistoryDtoActionEnum) {
    let tobdRecordHistoryDto: TobdRecordHistoryDto = data as TobdRecordHistoryDto;
    tobdRecordHistoryDto.tmrId = data.userId;
    tobdRecordHistoryDto.source = this.$t("getRecordPage_source").toString(); // 錄音調檔
    tobdRecordHistoryDto.action = action;

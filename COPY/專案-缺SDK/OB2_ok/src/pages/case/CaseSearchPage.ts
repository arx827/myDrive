...
@Component({
  components: { FblDataGrid, HiddenFolde, 
    CaseHistoryForm, MailRecord, QuestionAnswer, DragModal, UploadFileHistroy,InfRecord,MPlusHistory, CallupHistory, 
    HandleInfoForm, PostRecord,BlockHeader,QuestOpenArea,CustomerAnswer,QuestCheckIdArea,QuestArea,TeleResult}
})
export default class CaseSearchPage extends Vue {
  ...
  caseSearchPageGridModal:{[key:string]:boolean} = {
    idShowQuestPreviewModal:false, //問卷預覽顯示
  ...
  caseSearchPageGrid: FblPDataGridHolder<CaseSearchDto> = {
    ...
    columns: [
      ...
      {
        type: FblColumnType.TEMPLATE,  //為推上prod先改為PLAIN(不能點擊的)
        title: this.$t('case_search_grid_questName').toString(), //問卷名稱
        //property: "questName", //為推上prod先改為單純取問卷名稱(不能點擊的)
        width: 200,
        align: 'center',
        template:"questNameTemp",
      ...
      },
      {
        ...
        title: this.$t('case_search_grid_callFreqDay').toString(), //已電訪天數
        width: 100,
        align: 'center',
      },
      {
        ...
        title: this.$t('case_search_grid_callFreqVisit').toString(), //已電訪次數
        width: 100,
        align: 'center',
      },
      {
  ...
  arrangeCaseSearchCondition(){
    ...
    getCaseSearchCondition.closeStartDate = ValidationUtil.isEmpty(this.casePageSearchForm.closeStartString) ? null : moment(this.casePageSearchForm.closeStartDate).format("YYYY/MM/DD") ;
    getCaseSearchCondition.closeEndDate = ValidationUtil.isEmpty(this.casePageSearchForm.closeEndString) ? null : moment(this.casePageSearchForm.closeEndDate).format("YYYY/MM/DD") ;
  ...
  onCloseStartChange(date) {
    this.casePageSearchForm.closeStartString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(date)));
    this.isCloseDateStartVisible = false;
    this.isCloseDateEndVisible = false;
    this.feildValidate(this.casePageSearchValidateForm.closeStartDate, false, "success", "", "");
    this.feildValidate(this.casePageSearchValidateForm.closeEndDate, false, "success", "", "");
    this.validateSearch(false);
   }

   onCloseEndChange(date){
     this.casePageSearchForm.closeEndString = MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(date)));
     this.isCloseDateStartVisible = false;
     this.isCloseDateEndVisible = false;
     this.feildValidate(this.casePageSearchValidateForm.closeStartDate, false, "success", "", "");
     this.feildValidate(this.casePageSearchValidateForm.closeEndDate, false, "success", "", "");
  ...
  clearCloseStartDate(){
    ...
    this.feildValidate(this.casePageSearchValidateForm.closeEndDate, false, "success", "", "");
    this.isCloseDateStartVisible = false;
    this.casePageSearchForm.closeStartString = "";
    this.casePageSearchForm.closeStartDate = null;
  }
  clearCloseEndDate(){
    this.feildValidate(this.casePageSearchValidateForm.closeEndDate, false, "success", "", "");
    this.feildValidate(this.casePageSearchValidateForm.closeStartDate, false, "success", "", "");
    this.isCloseDateEndVisible = false;
    this.casePageSearchForm.closeEndString = "";
    this.casePageSearchForm.closeEndDate = null;
  }
  ...
  checkManualInputCloseStartDate(data: any) {
    this.isCloseDateStartVisible = false;
    this.isCloseDateEndVisible = false;
    this.feildValidate(this.casePageSearchValidateForm.closeStartDate, true, "", "", "");
    this.feildValidate(this.casePageSearchValidateForm.closeEndDate, true, "", "", "");
    const parseDate = this.formatter.parse(data.currentTarget.value);
    if (parseDate) {
        this.feildValidate(this.casePageSearchValidateForm.closeStartDate, false, "success", "", "");
        this.feildValidate(this.casePageSearchValidateForm.closeEndDate, false, "success", "", "");
        ...
        // 日期錯誤
        this.feildValidate(this.casePageSearchValidateForm.closeStartDate, true, "error", "hover", this.$t('global_dateError').toString());
    }
    this.casePageSearchForm.closeStartDate = parseDate ? parseDate : this.casePageSearchForm.closeStartDate;
    this.casePageSearchForm.closeStartString = parseDate ?
        MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(this.casePageSearchForm.closeStartDate.toString()))) :
        data.currentTarget.value.match(/\d+\/\d+\/\d+/)[0];
  }
  checkManualInputCloseEndDate(data: any){
    ...
    if (parseDate) {
        ...
        // 日期錯誤
        this.feildValidate(this.casePageSearchValidateForm.closeEndDate, true, "error", "hover", this.$t('global_dateError').toString());
    }
    this.casePageSearchForm.closeEndDate = parseDate ? parseDate : this.casePageSearchForm.closeEndDate;
    this.casePageSearchForm.closeEndString = parseDate ?
        MomentUtil.transformRocYearMonthDay(MomentUtil.default(new Date(this.casePageSearchForm.closeEndDate.toString()))) :
        data.currentTarget.value.match(/\d+\/\d+\/\d+/)[0];
  }
  ...
  eventMouseOverCloseStart(){
    if (this.casePageSearchValidateForm.closeStartDate.feedback) {
        this.isCloseDateStartVisible = true;
    } else {
        this.isCloseDateStartVisible = false;
    }
  ...
  eventMouseOverCloseEnd(){
    if (this.casePageSearchValidateForm.closeEndDate.feedback) {
        this.isCloseDateEndVisible = true;
    } else {
        this.isCloseDateEndVisible = false;
    }
    ...
    if (!this.isExportDisable) {
        if (this.caseSearchPageGrid.data.length == 0) {
            ...
            if(this.validateSearch(true)){
              var exportCaseSearchInDto:ExportCaseSearchInDto = {}
              exportCaseSearchInDto.caseSearchCondition = this.arrangeCaseSearchCondition();
              this.$caseSearchApi.exportCaseSearchLargeExcelInfoUsingPOST(this.caseSearchPageGrid.pagination.total.toString(), exportCaseSearchInDto)
              .then((resp)=>{
                  if(resp.data){
                    MessageUtil.messageSuccess(this.$t('case_search_large_excel').toString()); //匯出Excel超過案件查詢匯出筆數，Excel將改為信件寄出方式
                  }else{
                    // 打匯出 ajax
...
// ===================================== 驗證 ========================================================
  ...
  validateCloseStartDate(rule, value, callback){
    ...
    if (!ValidationUtil.isEmpty(value)) {
        ...
        if (parseDate) {
            ...
            this.isCloseDateStartVisible = true;
            this.feildValidate(this.casePageSearchValidateForm.closeStartDate, true, "error", "hover", this.$t('global_dateError').toString()); //日期錯誤
            callback(() => { });
        }
    }
    callback();
  ...
  validateCloseEndDate(rule, value, callback){
    ...
    if (!ValidationUtil.isEmpty(value)) {
        ...
        if (parseDate) {
            ...
            this.isCloseDateEndVisible = true;
            this.feildValidate(this.casePageSearchValidateForm.closeEndDate, true, "error", "hover", this.$t('global_dateError').toString()); //日期錯誤
            callback(() => { });
        }
    }
    callback();
 ...
 validateDueContactStartAndEndDate(rule, startDate, endDate, startString, endString, dateDisableStart, dateDisableEnd, callback) {
  ...
  questPreview(packNo,caseNo){
    ...
    this.caseSearchPageGridModal.idShowQuestPreviewModal=true;
    //開啟讀取
    LoadingUtil.show();

<template >
    <div tabindex="-1"  @keyup.enter="userUnlockSearch()">
            ...
            <template v-slot:hiddenArea="hiddenFoldeStyle">
                 <a-form-model
                   :label-col="{ xs: 7, md: 9, xxl: 7 }"
                    ...
                    :model="sendMPLUSTemplateSearchForm"
                ...
                <!-- 發送對象 -->
                    <a-form-model-item>
                        ...
                        <a-select
                        class="select"
                        v-model="sendMPLUSTemplateSearchForm.Target"
                        :allowClear="true"
                ...
                >
                    <a-input
                    type="text"
                    v-model="sendMPLUSTemplateSearchForm.content"
                    />
                </a-form-model-item>
                </a-col>
                </a-row>
                ...
                <div>
                    ...
                    <a-space style="margin-bottom: 16px">
                        <!-- 查詢 -->
                        <a-button type="primary" @click="sendMPLUSTemplateSearch()">{{
                        $t("global_search")
                        }}</a-button>
                        <!-- 清除 -->
                        <a-button type="default" @click="sendMPLUSTemplateSearchReset()">{{
                        $t("global_clean")
                        }}</a-button>
                    </a-space>
                    </a-row>
        ...
        <a-row>
            <a-col>
                ...
                @click="showSendMPLUSTemplateAddModal()"
                >
        ...
        <!-- 新增範本通知 -->
    <a-modal
      v-model="isSendMPLUSTemplateSettingVisible"
      :title="sendMPLUSTemplateSettingTitle"
      :closable="true"
      :maskClosable="false"
      :keyboard="false"
      :destroyOnClose="true"
    ...
    >
      ...
      :parentIsEmail = false
       :sendMode = "sendMode"
      @reloadSendMPLUSTemplate="isSendMPLUSTemplateSettingVisible = false;sendMPLUSTemplateSearch();"
      >
      
      </SendTemplateSettingUpdateForm>
       <template #footer>
        <!-- 離開 -->
        <a-button key="button" @click="isSendMPLUSTemplateSettingVisible = false">{{
          $t("global_leave")
        }}</a-button>
        <!-- 儲存 -->
        <a-button
...
<script src="./SendMPLUSTemplateSetting.ts" lang="ts"></script>
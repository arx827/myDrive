...
import { Option,SendTemplateMplusGrid,SendTemplateOptionsInit, PageOfSendTemplateMplusGrid } from "@fubonlife/obd-api-axios-sdk";
...
export default class SendMPLUSTemplateSettingPage extends Vue {
    ...
    sendMPLUSTemplateSettingFilter: FblFilters = {
        filters: []
    };
    ...
    */
    ...
    // 使用者資料顯示設定
    grid: FblPDataGridHolder<SendTemplateMplusGrid> = {
        rowKey: "templateId",
        data: [],
        pagination: {
        ...
        columns: [
            ...
            },
            {
                ...
                property: "createDate",
                formatter: (data: SendTemplateMplusGrid) => {
   ...
   * @param data 
   */
  deleteRecord(data:SendTemplateMplusGrid) {
    ...
    sendMPLUSTemplateSearch() {
        const target = FiltersUtil.setFilterParam("target", FblOperator.EQ, this.sendMPLUSTemplateSearchForm.Target);
        const content = FiltersUtil.setFilterParam("content", FblOperator.CONTAINS, this.sendMPLUSTemplateSearchForm.content);
        this.sendMPLUSTemplateSettingFilter = FiltersUtil.setFilters(target,content);
    ...
    //清除電仿照會碼搜尋結果
    sendMplusTemplateSearchReset() {
        this.sendMPLUSTemplateSearchForm = {
            Target: "", //發送對象
            content:"",//範本內文
        }
    }

    isSendMPLUSTemplateSettingVisible: boolean = false;
    sendMPLUSTemplateSettingTitle: string = "";
    updateSendTemplateSetting: SendTemplateMplusGrid = {};
    newTemplateId: string = "";
    sendMode: string="";

    //新增
    showSendMPLUSTemplateAddModal() {
        this.isSendMPLUSTemplateSettingVisible = true;
        this.sendMPLUSTemplateSettingTitle =  this.$t("sendTemplateSettingPage_create-sendTemplate").toString();
        this.updateSendTemplateSetting = {};
        this.sendMode = "M";
    }

    //編輯//刪除  
	onTableActionClick(e: FblActionEvent<SendTemplateMplusGrid>) {
    ...
    //編輯
    sendTemplateEdit(data:SendTemplateMplusGrid){
        this.isSendMPLUSTemplateSettingVisible = true;
        this.sendMPLUSTemplateSettingTitle = this.$t("sendTemplateSettingPage_update-sendTemplate").toString();
        this.updateSendTemplateSetting = data;
        this.sendMode = "M";
    }

    //刪除
    sendTemplateDelete(data:SendTemplateMplusGrid){
            Modal.confirm({
     ...
     reload() {
        ...
        if (this.searchFlag) {
            ...
            LoadingUtil.show();
            let sendMPLUSTemplatefilterStringtify = JSON.stringify(this.sendMPLUSTemplateSettingFilter);
            this.$sendTemplateApi.paginateSendMplusTemplateUsingPOST(
                this.grid.pagination.current - 1,
                this.grid.pagination.pageSize,
                sendMPLUSTemplatefilterStringtify
            ).then((resp: AxiosResponse<PageOfSendTemplateMplusGrid>) => {

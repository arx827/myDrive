...
import CustomerAnswer from "@/components/shared/customerAnswerPreview/CustomerAnswer.vue";
...
export default class QuestArea extends Vue {
  ...
  onTabChange(key) {
    ...
    if(questData.questCode == null && questData.overTwoQuestMessage == null){
      ...
      questData.questContDto.forEach( (qData) => {
        ...
        //客製化COL74顯示條件(都是單選題)
        if(this.form.coreSystemCode == "S074"){
          this.setDefaultTaskIdCol74(qData);
        }
  ...
  onQuestCheckClick(caseNo) {
    ...
    //2.確認是否全部的問卷檢核or拒答檢核是否完成且無其他訊息(需排除已結案件)
  ...
  onNoAnswerCheckClick(caseNo) {
    ...
    //確認是否全部的問卷檢核or拒答檢核是否完成且無其他訊息(需排除已結案件)
    this.questAllData.answer.questCheck[caseNo].pass = "Y";
    if(this.defaultPickupResult.casePackList == null){
      ErrorModalUtil.modalError( "系統發生錯誤，請聯絡系統管理員處理(casePackList is null)" );
      return;
  ...
  getDisplayOrNot(itemCode){
    ...
    //契約狀態
    let contract = this.defaultContract;
    ...
    if( itemCode == 'Z0002' || itemCode == 'Z0003' || itemCode == 'Z0004' || itemCode == 'Z0005' || itemCode == 'Z0013' ){
    ...
    if( itemCode == 'Z0007' || itemCode == 'Z0008' || itemCode == 'Z0010' || itemCode == 'Z0011' || itemCode == 'Z0012' || itemCode == 'Z0048' ){
    ...
    if( itemCode == 'Z0014' || itemCode == 'Z0015' || itemCode == 'Z0016' || itemCode == 'Z0026' || itemCode == 'Z0047'){
    ...
    if( itemCode == 'Z0018' ){
      ...
      if(rejectAnswer.includes(this.questAllData.answer.checkId)){
        return true;
      }
    }
    ...
    if( itemCode == 'Z0024' || itemCode == 'Z0025'){
      if(open){
        return true;
      }
      if(questHasCheckR && questNoCheckN && questCheck){
    ...
    if( itemCode == 'Z0027' ){
      if(contract == '催告' || contract == '自動墊繳' || contract=='停效' ){
        return true;
      }
    }

    if( itemCode == 'Z0028' ){
      if( this.checkForZ0028() ){
        return true;
      }
    }

    if( itemCode == 'Z0030'){
      if ( this.checkForZ0030() ){
        return true;
      }
    }
    ...
    if( itemCode == 'Z0032' ){
      ...
      for(itemN in checkNItem){
        if(this.questAllData.answer.quest[this.form.caseNo][checkNItem[itemN]] != undefined){
          ...
          if(this.checkAnswerIsN2(checkNItem[itemN])){
            return true;
          }
        }
      }
    ...
    if( itemCode == 'Z0033' ){
      ...
      for(itemN in checkNItem){
        if(this.questAllData.answer.quest[this.form.caseNo][checkNItem[itemN]] != undefined){
          if(this.checkAnswerIsY4(checkNItem[itemN])){
            return true;
          }
        }
      }
    ...
    if( itemCode == 'Z0035' ){
      if( allAnswerIsYes && questCheckOption.includes(this.questAllData.answer.questCheck[this.form.caseNo].doChecked) && !this.checkRiskCtrl('B0094') && !this.checkRiskCtrl('B0110')){
        return true;
      }
    }
    ...
    if( itemCode == 'Z0041' ){
      if( allAnswerIsYes && !questHasCheckR && this.checkRiskCtrl('B0094') && !this.checkRiskCtrl('B0110')){
        if(questCheckOption.includes(this.questAllData.answer.questCheck[this.form.caseNo].doChecked)){
          return true;
        }
      }
    ...
    if( itemCode == 'Z0043' || itemCode == 'Z0045' ){
      if( allAnswerIsYes && !questHasCheckR && this.checkRiskCtrl('B0110') && !this.checkRiskCtrl('B0094')){
        if(questCheckOption.includes(this.questAllData.answer.questCheck[this.form.caseNo].doChecked)){
          return true;
        }
      }
    ...
    if( itemCode == 'Z0049' ){
      if(questCheckOption.includes(this.questAllData.answer.questCheck[this.form.caseNo].doChecked)){
        ...
        if(this.questAllData.answer.quest[this.form.caseNo]["K0083"] != undefined){
          let answerK0083 = this.questAllData.answer.quest[this.form.caseNo]["K0083"].value;
          let answersK0083 = (answerK0083 == null)? []:answerK0083.split(":"); //複選會使用 ":" 分隔
          for( let val of answersK0083){
            if( val == "Y"){
              conditionOne = true;
            }
          }
        }
        if(this.questAllData.answer.quest[this.form.caseNo]["K0084"] != undefined){
          let answerK0084 = this.questAllData.answer.quest[this.form.caseNo]["K0084"].value;
          let answersK0084 = (answerK0084 == null)? []:answerK0084.split(":"); //複選會使用 ":" 分隔
          for( let val of answersK0084){
            if( val == "Y"){
              conditionOne = true;
            }
          }
        }
        if(this.questAllData.answer.quest[this.form.caseNo]["K0086"] != undefined){
          let answerK0086 = this.questAllData.answer.quest[this.form.caseNo]["K0086"].value;
          let answersK0086 = (answerK0086 == null)? []:answerK0086.split(":"); //複選會使用 ":" 分隔
          for( let val of answersK0086){
            if( val == "Y1" || val == "Y2" || val == "Y3"){
              conditionTwo = true;
            }
          }
        }
  ...
  checkAnswerIsN2(itemCode){
    let answer = this.questAllData.answer.quest[this.form.caseNo][itemCode].value;
    let answers = (answer == null)? []:answer.split(":"); //複選會使用 ":" 分隔
    for(let val of answers){
      if( val.startsWith("N2") ){
        return true;
      }
    }
    return false;
  ...
  checkAnswerIsY4(itemCode){
    let answer = this.questAllData.answer.quest[this.form.caseNo][itemCode].value;
    let answers = (answer == null)? []:answer.split(":"); //複選會使用 ":" 分隔
    for(let val of answers){
      if( val.startsWith("Y4") ){
        return true;
      }
    }
    return false;
  ...
  checkForZ0028(){
    var questCont;
    let itemCodeGroup = ["K0010", "K0011", "K0012", "K0013", "K0014", "K0028", "K0029", "K0030", "K0031", "K0032"];
    for(questCont in this.form.questContDto){
      if(this.form.questContDto[questCont].isShow && this.form.questContDto[questCont].answerOptionId != null){
        let answer = this.questAllData.answer.quest[this.form.caseNo][this.form.questContDto[questCont].itemCode].value;
        let answers = (answer == null)? []:answer.split(":"); //複選會使用 ":" 分隔
        if( itemCodeGroup.includes(this.form.questContDto[questCont].itemCode) ){
          for(let val of answers){
            if( val.startsWith("R") || val == "" ){
              return false;
            }
          }
        } else {
  ...
  checkForZ0029(){
    ...
    for(questCont in this.form.questContDto){
      if(this.form.questContDto[questCont].isShow && this.form.questContDto[questCont].answerOptionId != null){
        ...
        let answers = (answer == null)? []:answer.split(":"); //複選會使用 ":" 分隔
        if( !itemCodeGroup.includes(this.form.questContDto[questCont].itemCode) ){
          if(this.checkAnswerIsN(this.form.questContDto[questCont].itemCode)){
            conditionTwo = true;
          }
          if(conditionTwo){
            break;
  ...
  checkForZ0030(){
    ...
    for(questCont in this.form.questContDto){
      if(this.form.questContDto[questCont].isShow && this.form.questContDto[questCont].answerOptionId != null){
        ...
        let answers = (answer == null)? []:answer.split(":"); //複選會使用 ":" 分隔
        if( !itemCodeGroup.includes(this.form.questContDto[questCont].itemCode) ){
          if(this.checkAnswerIsN2(this.form.questContDto[questCont].itemCode)){
            conditionTwo = true;
          }
          if(conditionTwo){
            break;
  ...
  setTaskIdCol74(event){
    var questCont;
    if(event.target.name == "K0065" || event.target.name == "K0066"){
      if(event.target.value == "Y"){
        for(questCont in this.form.questContDto){
          if(this.form.questContDto[questCont].itemCode == "M0028"){
            this.form.questContDto[questCont].isShow = true;
          }
        }
      }else{
        for(questCont in this.form.questContDto){
          if(this.form.questContDto[questCont].itemCode == "M0028" || this.form.questContDto[questCont].itemCode == "M0027"){
            this.form.questContDto[questCont].isShow = false;
            this.form.questContDto[questCont].answer = '';
            this.questAllData.answer.quest[this.form.caseNo][this.form.questContDto[questCont].itemCode].value = '';
            this.radioAns[this.form.questContDto[questCont].itemCode] = "";
          }
        }
      }
    }
    if(event.target.name == "M0028"){
      if(event.target.value == "Y"){
        for(questCont in this.form.questContDto){
          if(this.form.questContDto[questCont].itemCode == "M0027"){
            this.form.questContDto[questCont].isShow = true;
          }
        }
      }else{
  ...
  setDefaultTaskIdCol74(qData){
    var questCont;
    if(qData.itemCode == "K0065" || qData.itemCode == "K0066"){
      for(questCont in this.form.questContDto){
        if(this.form.questContDto[questCont].itemCode == "M0028"){
          if(this.radioAns[qData.itemCode] == "Y"){
            this.form.questContDto[questCont].isShow = true;
          }else{
            this.form.questContDto[questCont].isShow = false;
          }

...
export default class CustomerAnswer extends Vue {
    ...
    get style():string {
        //控制保戶回答按鈕的位置
        //return (this.custType == "quest" || this.custType == "end")? "right:-10px;":""; //原始版本誤動
        //return (this.custType == "quest" || this.custType == "end")? "right:-10px;":"top:100px;";
        if(this.custType == "open"){
            return "top:100px ;";
            //return "right:-10px";
        }else if(this.custType == "checkId"){
            return "top:250px ;";
        }
        //return (this.custType == "checkId")? "top:100px;":"";
    }
    ...
    mounted() {
        ...
        if(document.getElementById("questDiv")!=null){ //如果questModal不為null則監聽scroll動作
            document.getElementById("questDiv").addEventListener("scroll", this.reset);
        }

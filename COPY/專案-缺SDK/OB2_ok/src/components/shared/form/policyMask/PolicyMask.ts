...
import { CheckShiftAndSkillInputDto, CheckShiftAndSkillOutputDto, ComponentDto, OutputErrorCodeDto, OutputErrorCodeDtoApiErrorCodeEnum, PendingGrid, PendingListDto, PolicyMarkDto, StaffDto } from "@fubonlife/obd-api-axios-sdk";
...
export default class UserSkillsSettingForm extends Vue {
    ...
    reload(){
        ...
        this.grid.data.forEach((row)=>{
            //系統日的判斷
            if(row.rocDateTime != null){
                if (new Date(row.rocDateTime) < new Date()) {
                    row.rocDateTime = "<font color='red'>"+row.rocDateTime+"</font>";
                } 
            }
        })
    ...
    grid: FblPDataGridHolder<PendingListDto> = {
        ...
        columns: [
            ...
            },
            {
                ...
                property: 'rocDateTime',  // 電訪期限
                title: this.$t('onDutyPage_telDeadline').toString(), 
                template: 'rocDateTimeTemp', //綁定.vue中的template樣式
                // formatter: (data: any) => {
                //     if (new Date(data.rocDateTime) < new Date()) {
                //         return "<font color='red'>" + data.rocDateTime + "</font>";
                //     } else {
                //         return data.rocDateTime;
                //     }
                // }
            },
            {
    ...
    //     if (new Date(record.rocDateTime) < new Date()) {
    //         // new Date(record.rocDateTime)
    //         return "red"
    //     } else {
    //         return "black"
    //     }
    ...
    policyMarkSave(){
        ...
        this.$pendingPageApi.savePolicyMarkUsingPOST(submitData).then((resp)=>{
            if(!resp.data.success){
                LoadingUtil.close();
                if(resp.data.returnObj!=null){
                    let returnInfo = JSON.parse(JSON.stringify(resp.data.returnObj));
                    Modal.error({
                        ...
                        content: (h) => {
                            if(returnInfo.length>0){
                                returnInfo.forEach(item => {
                                    ...
                                    }, },this.$t('pedding_packNo').toString() +  "：" + item.packNo + "、" + this.$t('pedding_policyNo').toString() +  "：" + item.policyNo )); //名單序號
                                })
                                //B0701
                                if(resp.data.returnMessage != null){
                                    let returnMessage = JSON.parse(JSON.stringify(resp.data.returnMessage));
                                    console.log(returnMessage);
                                    msgListWithPObject.push(h('div', returnMessage)); //保單號碼
                                    // returnMessage.forEach(item =>{
                                    //     msgListWithPObject.push(h('div', item)); //保單號碼
                                    // })
                                }
                                // msgListWithPObject.push(h('div', "已取件處理中，不可進行案件註記！")); //保單號碼
        ...
        Modal.confirm(
            {
                ...
                content: (h) => {
                    ...
                    msgListWithPObject.push(h('div', this.$t('pedding_tmr_not_onShift_1').toString())); //主此方便聯絡時段非指定的電訪員當班時段，請確認。
                    msgListWithPObject.push(h('div', this.$t('pedding_tmr_not_onShift_2').toString())); //點選「是」：重新指定電訪員
                    msgListWithPObject.push(h('div', this.$t('pedding_tmr_not_onShift_3').toString())); //點選「否」：案件將回到大眾池
                    return h('div', {}, msgListWithPObject);
                },
                onOk: () => {
                    //清空指定電訪員

                  ...
                  >
                    ...
                    >
                      <a-radio-group
                        ...
                        class="radio-group"
                      >
                        <template v-for="(item, index) in reasonOptList">
                          <div
                            v-if="item.label"
                            :key="index"
                          ...
                          >
                            ...
                            />
                            <div
                              class="d-sm-flex radio__input__block"
                              style="width:100%;"
                            >
                              <div
                                style="width:40px;"
...
@Action('setLoading') setLoading;
  ...
  onSubmit() {
  	(this.$refs.ruleForm as any).validate((valid) => {
  		if (valid) {
  			if (this.form.isPhyConsult === 2) {
  				const { isPhyConsult, unPhyConsultDesc, unPhyConsultReason } = this.form;
  				this.formRules.unPhyConsultReason = [{ required: true, message: '此欄位為必填', trigger: 'change' }];
  				(this.$refs.ruleForm as any).validate((valid) => {
  					if (valid) {
  						this.$emit('modalSubmit', {
  							isPhyConsult,
  							unPhyConsultReason,
  							unPhyConsultDesc: this.form.unPhyConsultReason !== 3
  								? this.reasonOptList.filter((el) => el.value == this.form.unPhyConsultReason)[0].label
  								: unPhyConsultDesc,
  						});
  					}
  				});
  			} else if (this.form.isPhyConsult === 1) {
  				this.$emit('modalSubmit', this.form);
  			}
  			// this.confirm();
  		} else {

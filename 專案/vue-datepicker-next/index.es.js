...
function Popup(originalProps, { slots }) {
...
function getCalendar({ firstDayOfWeek, year, month }) {
  const arr = []
...
function PickerInput(originalProps, { slots }) {
  const props = withDefault(originalProps, {
    editable: true,
    disabled: false,
    clearable: true,
  ...
  const isTW = locale2.twCanlender
  ...
  const text = computed(() => {
    ...
    if (Array.isArray(props.value)) {
      return props.value
        .map(v => {
          if (isTW) {
            const formatTmp = props.formatDate(v)
            const y = parseInt(formatTmp.substring(0, 4)) - 1911
            return `${y}${formatTmp.substring(4, formatTmp.length)}`
          } else {
            return props.formatDate(v)
          }
        })
        .join(innerSeparator.value)
      // return props.value.map(v => props.formatDate(v)).join(innerSeparator.value)
    }
    let textFormat = ''
    if (isTW) {
      const formatTmp = props.formatDate(props.value)
      const y = parseInt(formatTmp.substring(0, 4)) - 1911
      textFormat = `${y}${formatTmp.substring(4, formatTmp.length)}`
    } else {
      textFormat = props.formatDate(props.value)
    }
    return textFormat
  })
  ...
  return () => {
    ...
    const inputProps = __spreadProps(
      __spreadValues(
        {
          name: 'date',
          type: 'text',
    ...
    return createVNode(
      ...
      [
        ((_a = slots.input) == null ? void 0 : _a.call(slots, inputProps)) || createVNode('input', inputProps, null),
        showClearIcon
          ? createVNode(
              'i',
              {
              ...
              [((_b = slots['icon-clear']) == null ? void 0 : _b.call(slots)) || createVNode(render$1, null, null)],
            )
          : null,
        createVNode(
          'i',
...
const pickerInputBaseProps = keys()([
  'placeholder',
  'editable',
  'disabled',
  'clearable',
...
function Picker(originalProps, { slots }) {
  var _a
  const props = withDefault(originalProps, {
    prefixClass: 'mx',
    valueType: 'date',
  ...
  const formatDate = (date, fmt) => {
    ...
    // const date2 = `${date.getFullYear()}/${date.getMonth}`
  ...
  const renderSidebar = slotProps => {
    ...
    return createVNode(
      ...
      [
        (_a2 = slots.sidebar) == null ? void 0 : _a2.call(slots, slotProps),
  ...
  return () => {
    ...
    const footer =
      ...
      createVNode(
        ...
        [
          (_a2 = slots.footer) == null ? void 0 : _a2.call(slots, slotProps),
          confirm &&
            createVNode(
              'button',
              {
    ...
    return createVNode(
      ...
      [
        createVNode(
          PickerInput$1,
          __spreadProps(__spreadValues({}, pick(props, pickerInputBaseProps)), {
          ...
          pick(slots, ['icon-calendar', 'icon-clear', 'input']),
        ),
        createVNode(
          Popup$1,
          {
...
function TableHeader({ type, calendar, onUpdateCalendar }, { slots }) {
  var _a
  const prefixClass = usePrefixClass()
  const lastMonth = () => {
...
function TableDate({
  ...
  const locale2 = useLocale().value
  const isTW = locale2.twCanlender
  const { yearFormat, monthBeforeYear, monthFormat = 'MMM', formatLocale } = locale2
  ...
  const yearLabel = createVNode(
    ...
    [isTW ? calendar.getFullYear() - 1911 : formatDate(calendar, yearFormat)],
  )
  const monthLabel = createVNode(
    'button',
    {
      type: 'button',
                    ...
                    [
                      showWeekNumber &&
                        createVNode(
                          ...
                          [createVNode('div', null, [getWeekNumber(row[0])])],
                        ),
                      row.map((cell, j) =>
                        createVNode(
                          'td',
...
function TableMonth({ calendar, getCellClasses, onSelect, onUpdateCalendar, onUpdatePanel }) {
  const prefixClass = usePrefixClass()
  const locale2 = useLocale().value
  const isTW = locale2.twCanlender
        ...
        },
        {
          default: () => [
            createVNode(
              ...
              [isTW ? calendar.getFullYear() - 1911 : calendar.getFullYear()],
            ),
          ],
        },
      ),
...
const getDefaultYears = calendar => {
  ...
  const isTW = locale2.value.twCanlender
  const firstYear = isTW
    ? Math.floor((calendar.getFullYear() - 1911) / 10) * 10
    : Math.floor(calendar.getFullYear() / 10) * 10
  const years = []
  for (let i = 0; i < 10; i++) {
  ...
  return createVNode(
    ...
    [
      createVNode(
        ...
        {
          default: () => [
            createVNode('span', null, [firstYear]),
            createVNode(
              'span',
              {
            ...
            createVNode('span', null, [lastYear]),
          ],
        },
      ),
      createVNode(
...
function Columns({ options, getClasses, onSelect }) {
  const prefixClass = usePrefixClass()
...
function generateList({ length, step = 1, options }) {
...
function getFixedOptions({ date, option, format: format2, formatDate }) {
  const result = []
...
function TimeRange(originalProps) {
  ...
  const emitChange = (type, index2) => {
    var _a
    ;(_a = props['onUpdate:value']) == null
      ? void 0
      : _a.call(props, innerValue.value, type === 'time' ? 'time-range' : type, index2)
  }
...
function DateTimeRange(originalProps) {
  ...
  return () => {
    ...
    return createVNode(
      ...
      [
        createVNode(CalendarRange$1, calendarProp, null),
        timeVisible.value && createVNode(TimeRange$1, timeProps, null),
      ],
    )
  }
}
...
function DatePicker(originalProps, { slots }) {
    ...
    __spreadValues(
      {
        ...
        ['icon-calendar']: () =>
          type === 'time' ? createVNode(render, null, null) : createVNode(render$2, null, null),
      },
      slots,
    ),
  )

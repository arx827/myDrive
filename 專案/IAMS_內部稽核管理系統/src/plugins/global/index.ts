...
export class GlobalService implements PluginObject<GlobalSerivceOption> {
  ...
  public changeRouterAndaddParam({ toRouter, query, params }: {toRouter: string; query?: any; params?: any }, blank?: boolean) {
  	...
  	if (blank) {
  		...
  		this.opt.router.push({ name: routerName, params });
  	}
  }
  ...
  /**
	 ...
	 */
  public getCurrentRole(): RoleDto {
  	if (sessionStorage.getItem('currentRoleData')) {
  		const role: RoleDto = JSON.parse(sessionStorage.getItem('currentRoleData'));
  		return role;
  	}
  }
  ...
  public getCurrentRoleId(): string {
  	// console.log("sessionStorage.getItem('currentRoleData')", sessionStorage.getItem('currentRoleData'));
  	if (sessionStorage.getItem('currentRoleData') && sessionStorage.getItem('currentRoleData') !== 'undefined') {

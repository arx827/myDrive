      ...
      <div class="audit__sideWrap">
        ...
        >
          <li
            ...
            :class="{'audit__sideMenu__item--selected': item.operate === 'add'}"
          ...
          >
            ...
            />{{ item.auditItemName }}
            <!-- <button
              v-if="auditCatchArr && auditCatchArr.filter(i => i.auditItem == item.auditItem).length > 0"
              class="btn border-0 ms-auto"
              @click="onRemoveSideMenu(item.auditItem)"
            >
              <a-icon
        ...
        <template v-for="(item, index) in changeAuditArr">
          ...
          >
            <div class="audit__textarea__header">
              <span class="audit__textarea__title">{{ auditSideMenuArr.find(i => i.auditItem == item.auditItem).auditItemName }}</span>
              <!-- TODO: 此階段先隱藏 -->
              <!-- <a-form-model
              v-if="dataTypeStr == 'A' "
            ...
            <a-form-item
              ...
              :validate-status="getCheckValidation.view2.includes(item.auditItem) ? 'error' : ''"
            >
              <a-textarea
                ...
                :disabled="!isEdit"
                :auto-size="{minRows: 1 }"
                @change="clearCheckValidation({key: '2', auditItem: item.auditItem})"
              />
            </a-form-item>
          </div>
          <p
            v-if="getCheckValidation.view2.includes(item.auditItem)"
...
export default class ControlCurrentView_2 extends Vue {
	...
	@detailModule.Getter('getRelProcess_2') getRelProcess_2;
	...
	onAddSelectAudit(auditItem) {
		if (this.isEdit) {
			...
			if (thatAuditSideMenu.operate == 'modify') {
				this.delRelProcess({ relprocessId: 'relProcess_2', auditItem });
			}
		} else {
			return false;
		}
   ...
   */
	...
	onSaveSideMenu() {
  	...
  	this.$dataCollectApi.creatAuditorItemInDataCollectUsingGET(inputText)
  		.then((resp) => {
				// 清空輸入框
				this.addAuditInput = '';
				// 刷新 查核項目

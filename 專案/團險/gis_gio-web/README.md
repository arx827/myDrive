...
"dependencies": {
    // ...略...
    "@fubonlife/giiss-api-ng-sdk": "1.0.0-BETA",
    // ...略...
},
// ...略...
```
...
2. 直接打包為 WAR 檔 (部署在 JBOSS EAP, WAS)，請使用以下指令
    ...
    > 預設會使用 ArtifactId (`gis-gio-web`) 作為 context root，可以透過指令參數改為客製化的 context root: `mvn package -Pdev -DcontextRoot=my-context-root`
...
2. `domain.xml` 加入 Server Group:
    ...
    * 調整 `initial-host` 指向 Domain 的 JGroups Peer Discovery 位置
    ...
    ```xml
    <server-groups>
        <server-group name="gis-gio-web" profile="full-ha">
...
3. `host.xml`  的 **servers** 區塊，加入一至數台的 Servers，作爲 Server Group 的成員。
    ...
    * 調整適當的 `port-offset`
    ...
    ```xml
    <servers>
        <server name="gis-gio-web-dx287" group="gis-gio-web" auto-start="true">
            <paths>
                <path name="custom.jboss.server.log.dir" path="/jbossLog/server/gis-gio-web-dx287"/>
            </paths>

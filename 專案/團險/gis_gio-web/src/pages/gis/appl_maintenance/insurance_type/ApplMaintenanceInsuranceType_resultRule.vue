<template>
  <div class="applMaintenanceInsuranceTypeResultRule">
    ...
    <div class="main-HeaderInfo d-flex">
      <div class="info__item locker__item">
        ...
        <p class="info__item__sub">
          {{ resultParam.policyModel.policyNo }}-{{ resultParam.policyModel.policySeq }}
        </p>
      </div>
      <div class="info__item locker__item">
        <p class="info__item__title">
      ...
      <div class="info__item">
        ...
        <p class="info__item__sub">
          {{ $enum.getVal('searchInsurableAttr', resultParam.attributes) }}
        </p>
      </div>
    </div>
    <div class="main-EditGroup">
      ...
      <div class="main-EditGroup__body">
        <a-form-model
          ref="applMaintenanceInsuranceTypeResultRuleForm"
          :form="form"
          :model="form"
          :rules="rules"
        >
...
import { MaintenanceByUnderRulesQueryDto, MaintenanceOfInspectionConditionsDto } from '@fubonlife/co-giiss-api-axios-sdk';
import { Modal } from 'ant-design-vue';

@Component({
  components: { LayoutLoading },
})
export default class ApplMaintenanceInsuranceTypeResultRule extends Vue {
  h = this.$createElement;

  pageLoading = false;

  resultParam: MaintenanceByUnderRulesQueryDto = {
    attributes: '',
    insCode: '',
    policyModel: {
      policyNo: '',
  ...
  form: MaintenanceOfInspectionConditionsDto = {
    insCode: null,
    policyNo: null,
    policySeq: null,
    times: null,
  ...
  setEditParam() {
    ...
    if ($query) {
      this.resultParam = $query;
      // console.log(this.resultParam);
      const { insCode, attributes, policyModel: { policyNo, policySeq, times } } = this.resultParam;
      this.form = Object.assign(this.form, {
        insCode, attributes, policyNo, policySeq, times,
      });
    }
  }

  // API: 核保資料 查詢
  async getMaintenancebyunderrulesq() {
    this.pageLoading = true;
    await this.$gioInsPlanReviewApi
      .maintenanceByUnderRulesQueryUsingPOST(this.resultParam)
      .then((resp) => {
        if (resp.data.status == 200) {
          const getData = resp.data.data;
          const {
            unAdd, unAddCheck, famAddOnly, famAddCheck, minAge, maxAge, ageCheck, famRate, famRateCheck,
            exclItem1, exclItem2, exclItem3, exclItem4, exclItem5, exclItemCheck, coItem, coItemRate, coItemCheck,
            advAgeStart, advAgeMaxSa, advAgeCoItem, advAgeCoItemSa, advAgeCheck, hlNtcSa, hlNtcAge, hlNtcCoItem, hlNtcCoItemSa,
            hlNtcCheck, disNtcAge, disNtcSa, disNtcCoItem, disNtcCoItemSa, disNtcCheck, hivRpt, hivRptCheck, burNtcAge, burNtcAgeCheck,
          } = getData;
  ...
  onSubmit() {
    ...
    (this as any).$validateForm('applMaintenanceInsuranceTypeResultRuleForm')
    .then((res) => {
      // 擇一欄位 有 error
      ...
      this.$gioInsPlanReviewApi
        .maintenanceByUnderRulesUpdateUsingPOST(this.form)
        .then((resp) => {
          if (resp.data.status == 200) {
  ...
  async created() {
    this.setEditParam();
    await this.getMaintenancebyunderrulesq();
   ...
   */
  // TEST:
  // @Watch('mainEditGroupObject', { deep: true })

...
export default class ChangePassword extends Vue {
  ...
  formRules = {
  	...
  	oldpassword: [{ required: true, message: '請填入有效密碼' }, { min: 8, message: '密碼不符，請再次輸入' }, { max: 12, message: '密碼不符，請再次輸入' }],
  }
  ...
  onValChange(val) {
  	this.validates.forEach((element) => {
  		...
  		if (regex === 'custom') {
  			...
  			//  && /^(?!.*(\w)\1){2,}.+$/i.test(val) ? element.validate = true : element.validate = false;
  		} else {

<template>
  <div>
    ...
    <section class="page__section">
      ...
      <a-row type="flex">
        <h4 class="section__title list__count">財務與職業影像資訊：</h4>
      </a-row>
      <div
        class="precautions"
        v-if="
          getDeptSectionConfig.financialAndJobInfoPage
            .fnOccuImagesMessageTxtGroup.length > 0 && grid.data.length > 0
        "
      >
        ...
        <ol class="precautions__listGroup">
          <li
            ...
            v-for="(item, index) in getDeptSectionConfig.financialAndJobInfoPage
              .fnOccuImagesMessageTxtGroup"
            :key="index"
          >
            {{ item }}
          </li>
        ...
        >
          <template v-slot:template="slotProps">
            <a
              ...
              v-if="slotProps.data.imgIdxId && slotProps.data.fileName"
            >
    ...
    <section
      class="page__section"
      v-if="getDeptSectionConfig.financialAndJobInfoPage.isRepay50w"
    >
      <a-row type="flex">
        <h4 class="section__title list__count">
          {{sectionTitle}}
        </h4>
      </a-row>
      <div class="precautions">
        ...
        <ol class="precautions__listGroup">
          ...
          <li class="precautions__list__item">
            財務狀況是否顯著不相當
            <div>
              提醒審查事項：若為自行電訪，請一併檢附『電話訪談紀錄單』，將檔案轉為PDF檔，自行上傳到[佐證資料]。
            </div>
          </li>
        </ol>
      </div>
      <a-row type="flex" :gutter="[15, 16]">
        ...
        <a-col class="checkBlock" flex="1">
          <a-checkbox-group
            name="checkboxgroup"
            v-model="financialSourceB"
            style="width: 100%"
          >
            <a-checkbox
              v-for="(item, index) in financialSourceOpts"
              :disabled="isConfirm || !hasCaseAType"
              :class="{ nowrap: true, displayBlock: item.value === '5' }"
              :value="item.value"
              :key="index"
            >
              <span style="color: black; margin-right: 8px">
                {{ index + 1 }}.{{ item.label }}
              </span>
              <a-input
                v-model="financialSourceBOther"
                v-if="item.value === '5'"
                :disabled="
                  isConfirm || !hasCaseAType || financialSourceBOther_disabled
                "
          ...
          <a-row type="flex" :gutter="[15, 16]">
            <a-col :span="11">
              ...
              >
                <a-col :span="10">
                  <div>動產</div>
                  <div class="txt-small">(含動產、股票、基金)</div>
                </a-col>
                <a-col :span="14">
                  <a-input
    ...
    <section
      class="page__section"
      v-if="getDeptSectionConfig.financialAndJobInfoPage.isIntensiveTrading"
    >
      <a-row type="flex">
        <h4 class="section__title list__count">
          辦理「短期密集借還款」者，請
          <span class="mark-text">必填</span>
          (1)借款用途及(2)還款資金來源：
        </h4>
      </a-row>
      <div class="precautions">
      ...
      <div>
        ...
        <div class="checkBlock">
          ...
          >
            <a-checkbox
              v-for="(item, index) in debitUsedOpts"
              :class="{ nowrap: true, displayBlock: item.value === '6' }"
              :disabled="isConfirm || !hasCaseNineType"
              :value="item.value"
              :key="index"
            >
              <span style="color: black; margin-right: 8px">
              ...
              <a-input
                v-model="debitUsedOther"
                v-if="item.value === '6'"
                :disabled="
                  isConfirm || !hasCaseNineType || debitUsedOther_disabled
                "
      ...
      <div>
        ...
        <div class="checkBlock">
          ...
          >
            <a-checkbox
              v-for="(item, index) in debitSourceOpts"
              :class="{ nowrap: true, displayBlock: item.value === '6' }"
              :disabled="isConfirm || !hasCaseNineType"
              :value="item.value"
              :key="index"
            >
              ...
              <a-input
                v-model="debitSourceOther"
                v-if="item.value === '6'"
                :disabled="
                  isConfirm || !hasCaseNineType || debitSource_disabled
                "
                style="width: calc(100% - 81px)"
                :maxLength="60"
                type="text"
...
export default class FinancialAndJobInfoPage extends Vue {
  ...
  @FinanceJobInfo.Action("setFinanceJobinfoPageLoading")
  setFinanceJobinfoPageLoading;
  ...
  created() {
    ...
    const depId = sessionStorage["review_assignment_page"].substring(0, 2);
    if (depId == "VP" || depId == "RN") {
      this.noDataText = "查無資料";
    } else {
      this.noDataText = "查無5年資料";
    }

    if (depId == "VP") {
      this.sectionATitle = "辦理「保單還款（新台幣50萬元以上）」者，(1)資金來源-可以複選 或 (2)財務狀況，擇一填寫：";
    } else if (depId == "MC") {
      this.sectionATitle = "辦理「單筆增額（新台幣50萬元以上）」者請填本項，(1)資金來源-可以複選 或 (2)財務狀況，擇一填寫：";
    } else if (depId == "RN") {
      this.sectionATitle = "辦理「彈性繳費（新台幣50萬元以上）」者請填本項，(1)資金來源-可以複選 或 (2)財務狀況，擇一填寫：";
    }
  }
  ...
  watchFinancialSourceB(val) {
    this.financialSourceBOther_disabled =
      val && val.includes("5") ? false : true;
  }

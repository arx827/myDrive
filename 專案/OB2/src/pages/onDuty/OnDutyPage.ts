...
export default class OnDutyPage extends Vue {
  ...
  */
  ...
  created() {
    ...
    //照會/會辦未結案提醒處理
    this.notiInfNotCloseRemindProcess();
  ...
  //照會/會辦未結案提醒處理
  notiInfNotCloseRemindProcess() {
    this.$informApi.searchOpenCaseUsingGET(PackMatchModule.pickupResult.firstCasePack.packNo).then((resp) => {
  ...
  changeServeInfo(currentPackNo: CasePackDto) {
    ...
    if (!validationUtil.isEmpty(currentPackNo) && !validationUtil.isEmpty(currentPackNo.packNo)) {
      if (currentPackNo.casePolicyLogInfoList != null && currentPackNo.casePolicyLogInfoList.length > 0) {
        const caseNoList = currentPackNo.casePolicyLogInfoList.map(c => c.caseNo);
        if (caseNoList != null && caseNoList.length > 0) {
          const findCaseNosByPackNoPromise = this.$policyDataApi.getPolicyContractStatusDataUsingPOST(caseNoList).then(() => {
    ...
    for (let caseData of this.questAreaData.questDto) {
      ...
      for (let itemCode in this.questAllData.answer.quest[caseNoData]) {
        ...
        if( remark != undefined ){
          ...
          console.log(remarkValueArray);
          for(let remarkValueString of remarkValueArray){
            console.log(remarkValueString);

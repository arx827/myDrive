...
export interface tQuestItemFormModel {
  ...
  answerOption?: string;  //答案選項
  otherOption?: string;   //其他選項輸入
  rightsContent?: string; //權益信函內容
  questAnswer?: string;   //Q&A說明
  ...
  subItems?: Array<QuestionBankLetterRightSubDto>; //子題項清單
}

@Component({ components: { FblLevelSelect } })
export default class QuestionBankLetterRightsForm extends Vue {
  @Prop()
  ...
  formRules: { [key: string]: ValidationRule[] } = {
    itemTitleCode: [{required:true, validator:this.validationItemTitleCode, trigger:"blur" }],
  ...
  /** 子題項Form */
  formSub: QuestionBankLetterRightSubDto = {};
  ...
  reset() {
    ...
    //清除已選取子題項
    this.selectedSubItem.clear();

    if (this.isEditing) {
      //塞入已選取子題項
      for(let subItem of this.initData.subItems){
        this.selectedSubItem.set(subItem.subItemCode, subItem.subItem);
      }
      ...
      this.form = {
        ...
        createDate: "",
        updateDate: "",
        subItems: new Array<QuestionBankLetterRightSubDto>()
      };
  ...
  public submit() {
    ...
    //檢核子題項是否有資料
    if(this.selectedSubItem.size == 0 && this.isSubShow){
      Modal.error({
        title: "錯誤",
        content: "子題項尚未設定",
      });
  ...
  insertQuestItem(){
    ...
    .then((resp) => {
      if( resp.data == "100" as any ){
        ErrorModalUtil.modalError("題目編號已存在");
        return;
      }
  ...
  /** 已選子題項塞入Form */
  setFormSubItems(){
    this.form.subItems = new Array<QuestionBankLetterRightSubDto>();
    
    let seq = 0;
    this.selectedSubItem.forEach((value: string, key: string) => {
      this.formSub = {
      ...
      this.form.subItems.push(this.formSub);
    });
  }

  /** 開啟子題項設定窗 */
  openSubForm() {
    this.$emit("formOpenSub", true, this.selectedSubItem);
  }
   ...
   *        ATC10:子題問答複選, ATC11:母題問答單選null:無選項
  ...
  onAnswerTypeChange(type: string){
    ...
    const answerOptions = ["ATC01", "ATC02", "ATC03", "ATC06", "ATC07", "ATC08", "ATC09", "ATC10"];
    if( answerOptions.includes(this.form.answerTypeCode) ){
      this.isSubShow = false;
      this.isAnswerOptionEdit = true;
    }else if( this.form.answerTypeCode == "ATC04" || this.form.answerTypeCode == "ATC05" || this.form.answerTypeCode == "ATC11" ){
      this.isSubShow = true;
      this.isAnswerOptionEdit = true;
    ...
    if( this.isAnswerOptionEdit ){
      ...
      .then((resp) => {
        ...
        if(resp.data != null && resp.data.length != 0 && type != "load"){
          ...
          this.form.answerOption = resp.data[0].label;
        }
      }).catch((err) => {
        console.log(err);
      });
  ...
  /** 答案選項下拉異動 - 記錄選項中文  */
  onAnswerOptionIdChange(value, e){
    this.form.answerOption = e.componentOptions.children[0].text;

    //如答案類別為題組，清除已選取子題項 
    if( this.form.answerTypeCode == "ATC05" ){
      this.selectedSubItem.clear();
    }
  }
  ...
  validateSubmit(){
    ...
    this.validationItemTitleCode(null, this.form.itemTitleCode, () => {
      if(this.itemTitleValid.state == "error") vaild = false;
    });
  ...
  /** 檢核題目類型 - 為必填 */
  validationItemTitleCode(rule, value, callback){
    this.itemTitleValid.reset();

    if( validationUtil.isEmpty(value) ){
      this.itemTitleValid.msg = "請選擇題目類型";
    }

...
export default class AddAndEditModal extends Vue {
  ...
  allowUseSymbolFields = [
    this.$settingsEnum.columns.TITLE.value,
  ...
  searchAvailableNoticeSymbol() {
    ...
    this.$noticeApi.searchAllNoticeSymbolUsingGET()
      .then((res) => {
  ...
  insertNoticeSymbolTemplate(symbol) {
    ...
    let filedName = '';
    if (this.currentEditField === this.$settingsEnum.columns.TITLE.value) {
      filedName = 'noticeSubject';
      // 抓取textarea滑鼠框起來的部分插入文字
      const startPosition = ((((this.$refs.titleInput as any).$refs.resizableTextArea as any)).$refs.textArea as any).selectionStart;
      const endPosition = ((((this.$refs.titleInput as any).$refs.resizableTextArea as any)).$refs.textArea as any).selectionEnd;
      ...
      (this.$refs.titleInput as any).focus();
    }
  ...
  checkAndUpdateSelectedNoticeSymbol() {
    ...
    const updatedList = [];
    this.form.selectedNoticeSymbolList.forEach((constenSymbol) => {
      if (noticeSubject.includes(constenSymbol.noticeSymbolTemplate) || noticeContent.includes(constenSymbol.noticeSymbolTemplate)) {
        updatedList.push(constenSymbol);
      }
    });
  ...
  addInfo() {
    ...
    this.$noticeApi.addNoticeTemplateUsingPOST(dto)
      .then((res) => {
        const isSuccess = res.data.success;
        const message = res.data.message;
  ...
  editInfo() {
    ...
    this.$noticeApi.modifyNoticeTemplateUsingPOST(dto)
      .then((res) => {
        const isSuccess = res.data.success;
        const message = res.data.message;

...
import AddAndEditModal from '@/pages/NoticeSetting/AddAndEditModal.vue';
import IpkButton from '@/components/shared/buttons/IpkButton.vue';
import NoticeModal from '@/pages/NoticeSetting/NoticeModal.vue';
import SendModal from '@/pages/NoticeSetting/SendModal.vue';
...
export default class NoticeSettingDataInfo extends Vue {
  @Action('setLoading') setLoading;

  /**
   * data
   */
  functionName = 'NoticeSetting'

  noticeKind = this.$settingsEnum.noticeType.NOTICE.value
  ...
  advancedSearchSettings = { // 進階查詢設定
    ...
    noticeSubject: { label: '通知標題', placeholder: '請輸入', type: 'inputText' },
  }
  ...
  searchInfo() {
    ...
    this.$noticeApi.paginateNoticeTemplateUsingPOST(dto)
      .then((res) => {
  ...
  openCheckInfoModal(e) {
    this.setLoading(true);
    this.$noticeApi.searchNoticeTemplateDetailUsingGET(e.formData.noticeTemplateId)
      .then((res) => {
        const isSuccess = res.data.success;
        const message = res.data.message;
        const content = res.data.content;
  ...
  getActionType(e) {
    ...
    switch (actionType) {
      case this.$settingsEnum.constant.MODIFY.value:
        ...
        this.setLoading(true);
        this.$noticeApi.searchNoticeTemplateDetailUsingGET(e.rowData.noticeTemplateId)
          .then((res) => {
            const isSuccess = res.data.success;
            const message = res.data.message;
            const content = res.data.content;

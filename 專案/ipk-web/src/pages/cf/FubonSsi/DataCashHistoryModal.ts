...
import { CfFbSsiCashKeyPageDto } from '@fubonlife/ipk-api-axios-sdk';
...
export default class DataCashHistoryModal extends Vue {
  ...
  */
  ...
  currentCashHistoryDetail = {}; // [查詢] 紀錄查詢時輸入參數
  ...
  onValChange(val) {
    ...
    let dto = {
      ...
      bfAccount: this.isEmpty(val.editInfo.bfAccount) ? undefined : val.editInfo.bfAccount,
      pageNum: 1,
      pageSize: 10,
    };
  ...
  */
  ...
  searchHistory(dto) {
    // 紀錄目前機構資訊、頁碼、排序等資訊
    this.currentCashHistoryDetail = dto;
    // 歷程查詢
    this.setLoading(true);
    this.$fubonSsiApi.paginateCashHistUsingPOST(dto)
      .then((resp) => {
  ...
  handlePageChange(e) {
    // 輸入的資訊+頁碼變化 再打API
    let searchDto: CfFbSsiCashKeyPageDto;
    searchDto = {
      ...this.currentCashHistoryDetail,

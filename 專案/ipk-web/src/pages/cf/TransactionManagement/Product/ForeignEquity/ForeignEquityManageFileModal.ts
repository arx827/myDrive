...
export default class ForeignEquityManageFileModal extends Vue {
  ...
  */
  async created() {
    // 下拉選單
    this.counterpartyIdOption = await this.$cfCommon.getCounterpartyIdOption();
    // 設定初始資料
  ...
  */
  ...
  resetManageFile() {
    let manageFileForm: any = {
      ...
      // 預設選第一個
      counterpartyId: !this.isEmpty(this.counterpartyIdOption) ? this.counterpartyIdOption[0].value : undefined,
    };
  ...
  validateBeforeSaveAttachment(dto): any {
    ...
    return new Promise((resolve, reject) => {
      this.$foreignEquityApi.checkBeforeSaveAttachmentUsingPOST(dto)
      .then((res) => {
        resolve(res.data);
      })
      .catch((error) => {

...
export default class TxReviewForeignBondUnStructuredNotes extends Vue {
  ...
  */
  form = {
    ...
    cfStatus: [this.$cfEnum.cfStatusEnum.find((i) => i.key === '已送審').val], // 交易確認狀態 預設已送審
  ...
  */
  ...
  // ipkGrid 表格欄位轉換
  async changeGridColumns() {
    const nVString = await this.$cfEnum.getLabel('eventOption', this.advancedSearchForm.event);
    if (nVString === '換券') {
  ...
  // API: 進階查詢
  async searchTxReview(dto) {
    this.setLoading(true);
    await this.changeGridColumns();
    this.$txReview.exchangeTenderOfferSearchTxReviewInfoUsingPOST(dto)
    .then((res) => {
  ...
  searchTxReviewDetail(txCode: string) {
    ...
    // const submitData = {
    //   event: '14',
    //   txCode: 'TQFFI181205002_1',
    // };
    this.setLoading(true);
    this.$txReview.exchangeTenderOfferSearchTxReviewDetailUsingPOST(submitData)
    .then((res) => {
      const content = res.data.content;
      if (!this.isEmpty(content)) {
        // 整理前台成交資訊
  ...
  validateBeforeModifyCfStatus(dto) {
    ...
    return new Promise((resolve, reject) => {
      this.$txReview.exchangeTenderOfferCheckBeforeModifyCfStatusUsingPOST(dto)
      .then((res) => {
        resolve(res.data);
      })
      .catch((error) => {
  ...
  modifyCfStatus(dto) {
    this.setLoading(true);
    this.$txReview.exchangeTenderOfferModifyCfStatusUsingPATCH(dto)
    .then((res) => {
    ...
    Object.entries(other).forEach(([key, item], index) => {
      ...
      if (!this.isEmpty(this.other[key])) {
        switch (key) {
          ...
          case 'outIssueDate':
          case 'inIssueDate':
          case 'outMaturityDate':
          case 'inMaturityDate':
              this.other[key].key = this.isEmpty(item) ? '' : moment(item).format('YYYY/MM/DD');
              break;
          default:

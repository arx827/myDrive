...
export default class TxReviewForeignBondUnStructuredNotes extends Vue {
  ...
  */
  ...
  ssi: { [key: string]: CheckInfoModel } = { // v-model綁定及表單欄位名稱
    ...
    cpRemark: { key: undefined, label: '備註', type: 'inputText' },
  };
  ...
  */
  ...
  setSearchTxReviewDto() {
    let dto = {
      applyDate: this.isEmpty(this.advancedSearchForm.applyDate) ? undefined : moment(this.advancedSearchForm.applyDate).format('YYYMMDD'),
  ...
  searchTxReview(dto) {
    ...
    this.$txReview.nonstructureSearchTxReviewInfoUsingPOST(dto)
    .then((res) => {
  ...
  searchTxReviewDetail(txCode: string) {
    ...
    this.$txReview.nonstructureSearchTxReviewDetailUsingPOST({ txCode })
    .then((res) => {
      const content = res.data.content;
      if (!this.isEmpty(content)) {
        // 先判斷資產類別
        ...
        // 整理收付款資訊
        this.setSsiInitInfo(content.ssi, invCategoryCodeCompType);
        // 上傳附件
  ...
  validateBeforeModifyCfStatus(dto) {
    ...
    return new Promise((resolve, reject) => {
      this.$txReview.nonstructureCheckBeforeModifyCfStatusUsingPOST(dto)
      .then((res) => {
        resolve(res.data);
      })
      .catch((error) => {
  ...
  modifyCfStatus(dto) {
    this.setLoading(true);
    this.$txReview.nonstructureModifyCfStatusUsingPATCH(dto)
    .then((res) => {
  ...
  setMainInitInfo(main, invCategoryCodeCompType) {
    ...
    this.main.actCurrency.key = transferUtil.transferPrice(main.actCurrency); // 實際交割幣別
  ...
  setOtherInitInfo(other, invCategoryCodeCompType) {
    ...
    // console.log(invCategoryCodeCompType);
    // TEST:
    // console.log('other_原始 =>', other);
    // console.log('other_傳遞 =>', this.other);
  ...
  // 整理收付款資訊
  setSsiInitInfo(ssi, invCategoryCodeCompType) {
    if (this.isEmpty(ssi)) {
      return;
    }
    // TEST:
    // console.log('ssi_原始 =>', ssi);
    // console.log('ssi_傳遞 =>', this.ssi);

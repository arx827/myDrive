...
import DomesticCheckInfoModal from '@/pages/cf/TransactionManagement/TxReview/DomesticCheckInfoModal.vue';
...
export default class TxReviewForeignEquity extends Vue {
  ...
  */
  form = {
    ...
    cfStatus: [this.$cfEnum.cfStatusConstant.SUBMIT.val], // 交易確認狀態，預設已送審
  ...
  */
  ...
  searchTxReview(dto: TxReviewInfo) {
    ...
    this.$txReview.domesticBondSearchTxReviewInfoUsingPOST(dto)
    .then((res) => {
  ...
  searchTxReviewDetail(txCode: string) {
    this.setLoading(true);
    this.$txReview.domesticBondSearchTxReviewDetailUsingPOST({ txCode })
    .then((res) => {
      const content = res.data.content;
      if (!this.isEmpty(content)) {
        // 整理前台成交資訊
  ...
  validateBeforeModifyCfStatus(dto) {
    ...
    return new Promise((resolve, reject) => {
      this.$txReview.domesticBondCheckBeforeModifyCfStatusUsingPOST(dto)
      .then((res) => {
        resolve(res.data);
      })
      .catch((error) => {
  ...
  modifyCfStatus(dto) {
    this.setLoading(true);
    this.$txReview.domesticBondModifyCfStatusUsingPATCH(dto)
    .then((res) => {
  ...
  setMainInitInfo(main) {
    ...
    Object.entries(main).forEach(([key, item], index) => {
      ...
      if (!this.isEmpty(this.main[key])) {
        this.main[key].key = itemVal;
        if (key === 'tradeNpa' || key === 'tradePrice' || key === 'apDealAmount' || key === 'apAiAmount' || key === 'apTax' || key === 'paymentAmount') {
          this.main[key].key = transferUtil.transferPrice(itemVal);
        }
        if (key === 'isWi') {
          this.main[key].key = transferUtil.getSelectOption(this.$cfEnum.isWIOption, itemVal)?.label;
        }
        if (key === 'tradeDealYield') {
          this.main[key].key = itemVal ? `${itemVal}%` : '';
        }
      }
    });
  }

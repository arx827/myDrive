...
export default class ManageFileModal extends Vue {
  ...
  */
  ...
  getDtoData() {
    ...
    return {
      custodian: this[formTabId].bondKeeping, // 債券保管
      tradeDate: this[formTabId].transactionDate && moment(this[formTabId].transactionDate).format('YYYYMMDD'), // 交易日
      bsType: this[formTabId].transactionType, // 交易別
      txCode: this[formTabId].txCode, // 交易編號
    };
  }
  ...
  */
  ...
  searchTxCode(dto) {
    this.setLoading(true);
    this.$txDoubleReview.structureSearchTxCodeUsingPOST(dto)
    .then((res) => {
  ...
  searchAttachmentRelation(dto) {
    this.setLoading(true);
    this.$txDoubleReview.structureSearchAttachmentRelationUsingPOST(dto)
    .then((res) => {
      const message = res.data.message;
      const isSuccess = res.data.success;
      const content = res.data.content;

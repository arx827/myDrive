...
export default class ManageFileModal extends Vue {
  ...
  */
  ...
  @Watch('addManageFileForm.event', { deep: true })
  onAddEventChange(val) {
    const tradeDirectionEnum = val === '15' ? this.$cfEnum.tradeDirectionEnum : this.$cfEnum.tradeDirectionTenderEnum;
    this.selectorOption.transactionType = tradeDirectionEnum.map((i) => ({
      label: i.key,
      value: i.val,
    }));
  }

  @Watch('deleteManageFileForm.event', { deep: true })
  onDeleteEventChange(val) {
    const tradeDirectionEnum = val === '15' ? this.$cfEnum.tradeDirectionEnum : this.$cfEnum.tradeDirectionTenderEnum;
    this.selectorOption.transactionType = tradeDirectionEnum.map((i) => ({
      label: i.key,
      value: i.val,
  ...
  async created() {
    // 下拉選單
    this.selectorOption.transactionType = this.$cfEnum.tradeDirectionOption;
    this.selectorOption.event = this.$cfEnum.eventOption;
    this.selectorOption.bondKeeping = await this.$cfCommon.getCustodianOption();
    this.selectorOption.currency = await this.$cfCommon.getCurrencyOption(['TWD']);
    // 初始資料
  ...
  */
  ...
  getDtoData() {
    ...
    return {
      ...
      currency: this[formTabId].currency,
  ...
  */
  ...
  searchTxCode(dto) {
    this.setLoading(true);
    this.$txDoubleReview.structureSearchTxCodeUsingPOST_1(dto)
    .then((res) => {
  ...
  searchAttachmentRelation(dto) {
    this.setLoading(true);
    this.$txDoubleReview.exchangeTenderOfferSearchAttachmentRelationUsingPOST(dto)
    .then((res) => {
      const message = res.data.message;
      const isSuccess = res.data.success;
      const content = res.data.content;
  ...
  */
  ...
  resetManageFile() {
    this.addManageFileForm = {
      transactionDate: undefined, // 交易日
      transactionType: this.$cfEnum.tradeDirectionOption.map((data) => data.value), // 交易別
      event: '15', // 交易類別
      txCode: '', // 交易編號
      bondKeeping: [], // 債券保管
      currency: [], // 幣別
